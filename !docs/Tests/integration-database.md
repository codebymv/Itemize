...
The latest 2026-07-23 clean-schema checkpoint verifies 111 expected tables and 87 top-level migration markers. Marker discovery now includes multiline `runMigrationOnce` calls, which exposed one previously uncounted existing marker in addition to the new shared-revocation marker. The retained suite currently passes 479 of 493 tests; 14 booking cases use calendar windows that are no longer in the future relative to the test date and fail before the Nest suite starts. The isolated workspace-content GraphQL suite passes all 17 tests, including wireframe read/create/update/delete/share parity, stale revision rejection, tenant concealment, public capability read and old-token denial, token reuse/rotation, durable revocation plus shared/owner realtime worker delivery, mixed list/note/whiteboard/wireframe/vault position persistence, and cleanup on a disposable PostgreSQL schema.
