...
On 2026-07-17 the one-command run rebuilt the database, verified 95 expected tables and 66 top-level migration markers, passed all 27 legacy suites and 468 tests, then passed 5 NestJS GraphQL suites with 47 additional tests. Seven live Socket.IO cases include two-client public-room revocation plus end-session HTTP commit, active-visitor eviction, rejoin denial, post-end visitor/agent write denial, and transcript reads that preserve offline state; four GraphQL context cases prove current membership enforcement, contact cases cover contact/profile parity, and tag/pipeline cases prove canonical vocabulary behavior. Authenticated-form cases prove list/detail/field/count parity, default-field creation, serialized plan enforcement, omitted-versus-null settings, CSRF and tenant privacy, atomic conditional-field remapping, publish validation, duplication with fresh IDs, deterministic submission paging, and tenant-private form/submission deletion. Four onboarding cases prove empty/default reads, typed lifecycle mutations, atomic durable events, concurrent feature updates, user isolation, validation, and CSRF. Anonymous form retrieval and submission remain covered by the retained HTTP suite.
