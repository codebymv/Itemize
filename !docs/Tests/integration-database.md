...
On 2026-07-17 the one-command run rebuilt the database, verified 94 expected tables and 65 top-level migration markers, passed all 27 legacy suites and 462 tests, then passed 4 NestJS GraphQL suites with 35 additional tests. Four GraphQL context cases prove current membership enforcement, twenty contact cases cover contact/profile parity, and six tag/pipeline cases prove canonical vocabulary behavior. Five authenticated-form cases prove list/detail/field/count parity, default-field creation, serialized plan enforcement, omitted-versus-null settings, CSRF and tenant privacy, atomic conditional-field remapping, publish validation, duplication with fresh IDs, deterministic submission paging, and tenant-private form/submission deletion. Anonymous form retrieval and submission remain covered by the retained HTTP suite.
