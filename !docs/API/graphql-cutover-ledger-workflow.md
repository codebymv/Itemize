...
All 26 email-template, SMS-template, administrator-email, contact-delivery, and Twilio-webhook operations are assigned. The seven organization email-template management operations are implemented behind independent default-off read/mutation flags with PostgreSQL parity and frontend rollback coverage. Delivery and the remaining authenticated management operations still target GraphQL; Twilio callbacks remain signed HTTP with durable replay claims. Inbound tenancy comes from a globally unique organization-owned receiving number before the sender is matched inside that tenant. See [Messaging GraphQL cutover contract](contracts/messaging-graphql-cutover.md).
