...
| `POST /api/billing/webhook` | `SubscriptionWebhooksModule` | Exact raw-body Stripe verification, transactional event claim, serialized canonical Stripe state reads, minimal replay snapshots, leased tenant reconciliation, normalized subscription/audit writes, and idempotent notification delivery | Implemented behind the default-off `SUBSCRIPTION_WEBHOOKS_NESTJS_ENABLED` origin proxy with a signed-payload dual-runtime parity suite covering activation, limits, stale ordering, terminal transitions, and quarantine; the legacy reconciliation and notification workers keep draining the shared tables; operator tooling remains |
| `POST /api/email/webhook/resend` | `EmailWebhooksModule` | Raw-body Resend/Svix verification, durable delivery claim, occurrence-time ordering, tenant-ambiguity quarantine, leased pending reconciliation, and log/campaign/suppression transitions | Implemented behind the default-off `EMAIL_WEBHOOKS_NESTJS_ENABLED` origin proxy with a signed-payload dual-runtime parity suite and cross-runtime claim deduplication; the legacy reconciliation worker keeps draining the shared table until the scheduler moves |
| `POST /api/sms-templates/webhook/status` | `SmsWebhooksModule` | Twilio form-signature verification, durable state claim, normalized transition and replay tests | Implemented behind the default-off `SMS_WEBHOOKS_NESTJS_ENABLED` origin proxy with a dual-runtime parity suite and cross-runtime claim deduplication |
| `POST /api/sms-templates/webhook/inbound` | `SmsWebhooksModule` | Twilio verification, durable message claim, globally unique organization-owned receiving-number routing, tenant-local sender matching, atomic conversation/message/log, and replay/quarantine tests | Implemented behind the same default-off origin proxy, which forwards raw form bytes plus the original protocol/host so URL-bound signatures validate upstream; operator tooling remains |
| `GET /api/social/webhook` | `SocialWebhooksModule` | Constant-time challenge-token comparison and fail-closed configuration tests | Implemented behind the default-off `SOCIAL_WEBHOOKS_NESTJS_ENABLED` origin proxy |
| `POST /api/social/webhook` | `SocialWebhooksModule` | Exact-body HMAC verification, normalized batch claim, bounded inline processing, leased overflow work, Facebook/Instagram identity separation, unmatched/ambiguous reconciliation, post-commit socket emission, and PostgreSQL concurrency/replay tests | Implemented behind the same default-off origin proxy with a signed-payload dual-runtime parity suite; the NestJS receiver claims durably without inline processing, and the legacy scheduler worker (which owns the Socket.IO host) processes and emits until the scheduler moves |
| `POST /api/webhooks/:workflowId` | `WorkflowWebhooksModule` | Per-workflow HMAC, expiry window, durable delivery claim and replay tests | Implemented behind the default-off `WORKFLOW_WEBHOOKS_NESTJS_ENABLED` origin proxy with a signed-payload dual-runtime parity suite and cross-runtime replay deduplication; the retained validation middleware's zod-v4 regression (HTML 400s) was repaired in the same change; execution worker remains |
