...
Current executable evidence covers explicit membership resolution, default fallback, outsider denial, malformed-ID rejection before PostgreSQL, missing-auth rejection, role enforcement, optional-context behavior, and connection release on failure. The legacy organizations suite passes against disposable PostgreSQL. The Phase 1 NestJS guard chain adds focused GraphQL operation tests plus 4 real-PostgreSQL cases proving default membership, current-role re-read, outsider denial, and immediate denial after membership deletion. A staging boundary rehearsal additionally queried the same search under two valid memberships: the default organization returned zero rows and the explicitly selected temporary organization returned its single owned row. The frontend still lacks an organization-switching control and always calls `ensure-default`, so user-driven switching remains a product and browser-test gap.
