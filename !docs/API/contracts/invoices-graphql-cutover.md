...
Settings, logo deletion/storage lifecycle, email preview, payment-link creation, estimate send, and most recurring behavior still lack sufficient executable characterization. Estimate conversion and manual generation are `characterizing`, but explicit request idempotency and real-PostgreSQL concurrent execution remain open. The next database-only billing gate is recurring-template CRUD; provider and deployment configuration remains deferred to the final cutover phase.
