...
- Logo deletion clears the visible tenant-owned reference and records its exact prior server-owned URL in one transaction. A leased worker retries local/S3 cleanup; an unavailable provider cannot turn a committed removal into a false failure, and an enqueue failure cannot leave a dangling cleared reference.
...
The product catalog is permanently cut over. `ProductsModule` owns typed organization-scoped list/create/update/delete operations with strict paging, literal search, deterministic ordering, decimal-string prices/tax rates, currency and recurring-period validation, nullable-field clearing, CSRF, and tenant-hidden misses. The frontend maps the typed schema into the existing product shape and calls all four GraphQL operations unconditionally; the former read and mutation rollout flags, Express product subrouter, and all four REST handlers are removed. The mixed invoice router exits `/products` before generic invoice `/:id` handlers can capture it, and explicit canaries require every retired method/path to return `404` without entering authentication. Fresh PostgreSQL now verifies the authoritative GraphQL/PostgreSQL contract directly, including decimal fidelity, literal metacharacter search, validation, CSRF, nullable clearing, and cross-tenant denial.

Invoice business profiles are permanently cut over. `InvoiceBusinessesModule` owns typed active-list, detail, create, partial-update, soft-delete, and atomic logo-removal operations. Lists retain last-used/newest ordering, inactive rows remain available by ID for historical references, blank optional fields normalize to null, logo URLs are server-owned, and tenant-hidden misses plus CSRF are enforced. The frontend calls all six GraphQL operations unconditionally; the former read and mutation rollout flags and six Express JSON/state handlers are removed. The retained Express business subrouter contains only multipart upload, followed by a `/businesses` retirement barrier that prevents generic invoice `/:id` handlers from capturing retired paths. Fresh PostgreSQL verifies authoritative GraphQL/PostgreSQL ordering, soft deletion, nullable normalization, tenant isolation, server-owned logos, durable logo cleanup, validation, and CSRF. `POST /api/invoices/businesses/:id/logo` remains bounded multipart HTTP.
...
Core invoice CRUD is permanently cut over. `InvoicesModule` owns organization-scoped list/detail queries plus create/update/delete mutations. Reads provide status/contact/literal-search filters, stable paging, and tenant-qualified contact, business, item, and payment projections. Creation validates tenant-owned references, allocates numbers atomically, and commits line items plus PostgreSQL-calculated tax, discount, total, and balance values in one transaction. Updates lock the row, permit only draft/sent edits, validate the effective date/reference state, and replace items plus derived totals atomically. Deletion conceals foreign rows and returns stable deleted identity. The frontend calls all five GraphQL operations unconditionally; both rollout flags, all five Express handlers, and the orphan CRUD route module are removed. Explicit canaries require those five method/path combinations to return `404` while proving PDF, send, payment-link, and record-payment paths still enter authentication. Fresh PostgreSQL verifies exact fractional calculations, direct database persistence, CSRF, tenant isolation, locked paid-invoice edits, conversion interoperability, and concurrent unique numbering.
...
Invoice settings are permanently cut over. `InvoiceSettingsModule` owns the selected-organization query, strict partial update, and atomic logo-removal operations. Reads return stored configuration or virtual retained defaults without creating a row; Stripe connection fields and the server-owned logo URL remain read-only. Writes normalize clearable fields, require CSRF, create a retained-compatible base row only when needed, and serialize on the same settings row used by atomic invoice-number allocation. Counter regression and exact formatted-number collisions fail with stable conflict reasons. The frontend calls all three GraphQL operations unconditionally; the former read and mutation rollout flags and three Express state handlers are removed. The retained Express settings subrouter contains only multipart upload, followed by a `/settings` retirement barrier that prevents generic invoice `/:id` handlers from capturing retired paths. Fresh PostgreSQL directly verifies tenant isolation, server-owned logo retention, validation, collision denial, durable logo cleanup, and concurrent settings/allocation convergence; focused service tests verify virtual defaults without row creation. `POST /api/invoices/settings/logo` remains bounded multipart HTTP.

Invoice email preview is permanently GraphQL-owned through `previewInvoiceEmail` in `InvoicesModule`. It requires CSRF and verified selected-organization membership even though it performs no database write. Message and subject are bounded plain text, the rendered document uses only the server-configured frontend origin, payment-link preview is inert, and no unsubscribe control is added to the transactional template. The frontend calls the mutation unconditionally and never sends the former client-selected base URL; the rollout flag, Express handler, and route module are removed. A dedicated `/email` retirement barrier prevents the old path from reaching generic invoice handlers. Both the inline preview and “Open in New Window” paths keep returned markup inside a script-disabled iframe. Fresh PostgreSQL proves layout semantics, membership and CSRF denial, stable validation, and hostile-markup escaping.

Estimate sending is implemented as the `sendEstimate` mutation in `EstimatesModule`. It requires CSRF, a tenant-owned draft/sent estimate with a recipient, and a bounded caller idempotency key. A locked transaction snapshots the recipient and rendered business/estimate inputs into `estimate_email_deliveries` before any provider call. A leased delivery attempt gives Resend a stable intent-derived idempotency key; confirmed delivery atomically records provider evidence and changes a draft to sent, while a definite rejection remains retryable and an ambiguous network outcome is quarantined for reconciliation without changing the estimate. Exact request replay returns the durable result without another provider call. The `jobs:estimate-email-deliveries` one-shot command drains due retries, and an independent default-off frontend flag preserves REST rollback. Fresh PostgreSQL proves success and stored evidence, exact replay, provider failure and worker recovery, ambiguous quarantine, CSRF, tenant isolation, validation, and terminal-state denial. The retained route's false-success behavior when email is unavailable is intentionally not preserved.

Invoice sending is implemented as the `sendInvoice` mutation in `InvoicesModule`. It requires CSRF, a tenant-owned eligible invoice with a recipient, bounded custom subject/message and CC values, an explicit resend choice, and a bounded caller idempotency key. Migration `035_invoice_email_deliveries` stores the immutable invoice/settings/options snapshot before external work and permits only one unresolved intent per invoice. Reusing a key with different content conflicts, and a new key cannot bypass a retry or ambiguous reconciliation hold. Optional Stripe Checkout creation and Resend delivery use intent-derived idempotency keys, and the transitional PDF provider renders the existing authorized invoice template as a required attachment. A requested payment link or PDF failure cannot degrade into a successful email without that content. Only confirmed email delivery atomically stores provider evidence and changes a draft invoice to sent; definite failures remain retryable, ambiguous Stripe/email outcomes require reconciliation, and exact replay cannot duplicate delivery. The `jobs:invoice-email-deliveries` one-shot command drains due retries. An independent default-off frontend flag preserves REST rollback and maps only confirmed delivery back into the retained invoice response. Fresh PostgreSQL proves the migration, CSRF, private foreign IDs, success and evidence, payment-link persistence, attachment/options forwarding, exact and conflicting replay, unresolved-intent bypass denial, definite failure, PDF failure, and ambiguous quarantine while every unconfirmed path leaves the invoice unsent. The retained route's false-success and silent attachment/link degradation are intentionally not preserved.

Standalone payment-link creation is implemented as `createInvoicePaymentLink` in `InvoicesModule`. Migration `036_invoice_payment_link_intents` records the tenant, invoice, exact amount/currency snapshot, request identity, and provider evidence before Stripe work, with database-enforced tenant consistency and one unresolved intent per invoice. The mutation requires CSRF, conceals foreign invoices, rejects terminal or zero-balance invoices, gives Stripe an intent-derived idempotency key, and returns only a confirmed URL. Exact replay returns the stored result without another provider call; stale reuse of a key conflicts; definite Stripe rejection permits a fresh request; ambiguous outcomes fence new keys for reconciliation. After Stripe responds, the transaction re-locks the invoice and quarantines the intent without exposing or persisting the URL if its balance, currency, or payable state changed. Existing open Checkout Sessions are reused by the shared provider only when tenant, invoice, exact amount, and currency match. `VITE_INVOICE_PAYMENT_LINK_GRAPHQL` is independently default-off and preserves the retained `{ url, session_id }` shape. Fresh PostgreSQL proves CSRF, tenant concealment, paid-state denial, durable success evidence, exact replay, stale-key conflict, definite retry, ambiguous fencing, and the post-provider balance race.

Business and settings logo deletion are implemented as `removeInvoiceBusinessLogo` and `removeInvoiceSettingsLogo`. Migration `037_invoice_logo_deletion_jobs` stores the exact tenant-bound cleanup receipt in the same transaction that clears the visible URL. A bounded leased worker first preserves any URL still referenced by another tenant row, then deletes only safe local logo names or exact keys in the configured S3 bucket's `logos/` prefix. Terminal receipts can be requeued if a formerly shared URL loses its final reference. Missing local objects are idempotent success, provider failures retry with backoff, and unsupported or foreign URLs dead-letter without storage access. Fresh PostgreSQL proves CSRF, tenant isolation, shared-reference preservation/requeue, exact receipts, replay behavior, worker completion, and rollback when receipt creation fails. Both removal operations are permanently GraphQL-owned, while both multipart uploads remain HTTP. Scheduling the one-shot cleanup and delivery commands remains deferred deployment configuration.

Invoice PDF delivery is implemented in `InvoicesModule` as retained authenticated HTTP at `GET /api/invoices/:id/pdf`, not as a GraphQL field carrying binary data. Cookie authentication and the selected organization are resolved before a tenant-scoped invoice, ordered items, business, and payment-settings snapshot reaches the shared legacy-compatible renderer. Invalid and foreign identities are concealed as not found, renderer unavailability is distinguished from generation failure, and non-PDF output fails closed. Successful responses use a sanitized attachment name, exact content length, `private, no-store`, `application/pdf`, `nosniff`, and a sandbox CSP. The legacy origin can proxy the same public URL to NestJS only when `INVOICE_PDF_NESTJS_ENABLED=true`; otherwise the request falls through to Express, so rollback does not require a frontend rebuild. Both previously inert invoice-page Download PDF controls now use that stable URL and revoke their temporary browser object URL. Fresh PostgreSQL proves authentication, malformed-ID handling, tenant isolation, snapshot scope, bytes and headers, unavailable rendering, and invalid-output failure; focused proxy and frontend tests prove safe forwarding, default-off fallback, download activation, and filename sanitation.

The signed Stripe invoice callback is implemented in `InvoiceWebhooksModule` as retained HTTP at `POST /api/invoices/webhook/stripe`. NestJS captures the exact request bytes and the controller verifies them with Stripe's SDK before interpreting the event; a missing signing secret, absent signature, altered body, or missing raw capture fails closed. The repository transaction claims the event and payment reference, locks the invoice, treats the invoice row as the tenant authority, inserts the payment, performs exact balance/status arithmetic in PostgreSQL, and inserts one durable `invoice_paid` workflow trigger. A domain-write failure rolls the event claim back, allowing a corrected delivery with the same event ID to succeed. Concurrent duplicates converge on one payment, one balance mutation, and one trigger. `STRIPE_INVOICE_WEBHOOK_NESTJS_ENABLED=true` makes the legacy origin proxy the unchanged public URL with only the raw body, signature, content type, and request ID; false or absent keeps the Express receiver. The retained verifier now also uses its captured raw bytes. No rollout configuration is enabled by this checkpoint.

Business and settings logo uploads are implemented as retained NestJS multipart HTTP at their existing URLs. Cookie authentication, selected-organization membership, and double-submit CSRF are verified before Multer accepts one `logo` part. The parser limits the file to 2 MiB and bounds files, fields, and parts; the service ignores the filename extension and requires structurally complete PNG, JPEG, GIF, or WebP bytes before choosing a server-owned name and type. New bytes are stored before a tenant-qualified row lock and metadata update; replacement atomically enqueues the exact prior URL for the existing leased cleanup worker. A failed database replacement compensates by deleting the newly stored object. Foreign businesses are concealed before storage access. `INVOICE_LOGO_UPLOADS_NESTJS_ENABLED=true` proxies only the two unchanged multipart URLs through the legacy origin; false or absent leaves both on Express. Fresh PostgreSQL proves both upload scopes, cleanup receipts, CSRF, structural rejection, tenant isolation, and retained response shapes. Local storage is development/test-only; production fails closed unless the shared S3 provider is configured.
