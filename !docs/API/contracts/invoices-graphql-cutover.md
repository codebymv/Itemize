...
Settings, logo deletion/storage lifecycle, email preview, payment-link creation, estimate send, and recurring generation behavior still lack sufficient executable characterization. Estimate conversion and manual generation are `characterizing`, but explicit request idempotency and real-PostgreSQL concurrent execution remain open. The next database-only billing gate is recurring generate-now idempotency and concurrent repeat handling; provider behavior and deployment configuration remain deferred.
