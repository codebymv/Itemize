...
Settings, logo deletion/storage lifecycle, email preview, payment-link creation, estimate send, and scheduled recurring generation behavior still lack sufficient executable characterization. Estimate conversion remains `characterizing`; the next database-only billing gate is its GraphQL conversion mutation with repeat and concurrent-repeat proof. Provider behavior and deployment configuration remain deferred.
