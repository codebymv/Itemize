...
Invoice email preview is implemented as the `previewInvoiceEmail` mutation in `InvoicesModule`. It requires CSRF and verified selected-organization membership even though it performs no database write. Message and subject are bounded plain text, the rendered document uses only the server-configured frontend origin, payment-link preview is inert, and no unsubscribe control is added to the transactional template. The adapter omits the legacy client-selected base URL and is independently default-off. Both the inline preview and the retained/GraphQL “Open in New Window” path keep returned markup inside a script-disabled iframe, so the rollback renderer cannot turn user content into an executable top-level document. Fresh PostgreSQL proves retained layout semantics, membership and CSRF denial, stable validation, and hostile-markup escaping.
