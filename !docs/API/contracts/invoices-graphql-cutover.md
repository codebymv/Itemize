...
Existing integration suites characterize core invoice CRUD, tenant isolation, tax/discount behavior, sequential numbering, edit restrictions, manual payments, sending, estimate CRUD, estimate calculations, and list pagination. Focused tests now prove that invoice numbers use one atomic PostgreSQL upsert, estimate MAX+1 allocation is serialized per organization, estimate conversion locks and scopes its source, manual recurring generation locks and scopes its template, and competing background runners skip an already claimed template. Real-PostgreSQL scenarios now exercise concurrent invoice creation and concurrent estimate conversion; like the Stripe concurrent-delivery scenario, they await an environment with Docker/PostgreSQL.

Businesses, products, settings, logo lifecycle, payment listing, email preview, payment-link creation, estimate send, and most recurring behavior still lack sufficient executable characterization. Estimate conversion and manual generation are now `characterizing`, but explicit request idempotency and real-PostgreSQL concurrent execution remain open. The next billing gate is to add real-PostgreSQL characterization for these high-risk operations before implementing their NestJS services or final GraphQL types.
