...
Authenticated enable/disable operations are GraphQL-only. Public capability reads remain rate-limited HTTP because they are link-oriented, unauthenticated protocols and must work without a GraphQL client or session. The ten authenticated list, note, whiteboard, wireframe, and vault sharing REST operations are no longer registered; the five public capability reads remain explicit HTTP boundaries.

`WorkspaceContentModule` owns the shipped list, note, whiteboard, and wireframe enable/disable mutations. `VaultModule` owns vault issuance/revocation. NestJS `PublicSharingModule` implements all five retained public capability reads, including the vault read; the legacy origin routes them through a default-off proxy (`PUBLIC_SHARING_NESTJS_ENABLED`) that falls through to the retained legacy routers until cutover. Active Canvas and Contents consumers use GraphQL without REST fallback, while `/shared/:kind/:token` remains the intentional read-only public-link HTTP boundary.
...
Disable atomically sets `is_public = false`, clears `share_token`, and clears `shared_at`. Re-enabling always issues a different token. The former implementation retained the token and resurrected a revoked URL on re-share; the GraphQL mutations now clear it.
...
Fresh PostgreSQL coverage proves the retained public reads for all five object types, malformed-token denial, nested whiteboard sanitization, note/list markup sanitization, privacy headers, and complete vault decrypt-failure closure. Nest workspace and vault coverage proves stable concurrent issuance, owner-only mutation, enable/reuse/disable/re-enable lifecycles, token clearing and rotation, locked-vault refusal, password-triggered vault revocation, and durable non-vault revocation outbox delivery. The full Express composition separately proves all former authenticated share/unshare methods return 404. Live Socket.IO coverage proves two active viewers receive a redacted revocation event and are evicted. Focused frontend tests prove every authenticated sharing transport and mutation identity with no application-data REST fallback. A dual-runtime fresh-PostgreSQL parity suite (`backend-v2/test/integration/public-sharing.integration-spec.ts`) proves the NestJS `PublicSharingModule` and the legacy routers return identical status codes, privacy headers, and bodies for all five kinds across active, malformed, unknown, and revoked tokens, plus locked-vault denial, vault item decryption, complete decrypt-failure closure, and the v2 snapshot projection; the origin's default-off `PUBLIC_SHARING_NESTJS_ENABLED` proxy preserves legacy rollback and is covered by focused proxy tests.
