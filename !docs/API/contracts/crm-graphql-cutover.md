...
`contactProfile` aggregates contact, invoices, signatures, payments, communications, activities, notes, lists, tasks, and bookings. Every collection now has its own retained-legacy bound, deterministic ID tie-breaker, total/truncation metadata, and `AVAILABLE`/`UNAVAILABLE` status. The parent contact is tenant-private and required; a child query failure preserves the healthy sections while marking only that section unavailable. An exhaustive focused matrix independently fails each of the nine child queries and proves that the other eight remain available; the service matrix proves that every failed section returns explicit `UNAVAILABLE`, empty nodes, zero total, and no false truncation. The legacy aggregator silently replaces a failed child query with an empty array, making "none" indistinguishable from "failed". Fresh-PostgreSQL characterization proves this defect for the stale payment query (`p.date`) and nonexistent list junction tables; GraphQL uses the current payment timestamps and direct organization-qualified contact links instead.

The generated consumer ledger and a source-scanning regression both prove that no shipped frontend calls the aggregate REST route or the `contactProfile` GraphQL field. The contact detail page intentionally composes the narrower contact, activity, and related-content operations. `contactProfile` therefore remains server-only with no browser transport flag; introducing a consumer requires removing that regression deliberately and completing a browser/rollback gate for the new experience.
...
Focused follow-up coverage on 2026-07-17 closed the aggregate-profile failure and consumer gate. Nine repository injections prove query-level isolation for invoices, signatures, payments, activities, notes, lists, communications, tasks, and bookings; nine service injections prove the public collection health shape for each failure. A backend contract regression verifies both zero generated-ledger consumers and no aggregate-profile reference in non-test frontend source. The full NestJS suite passes 93 tests, and the fresh runner still passes 464 legacy plus 43 NestJS integration cases with disposable Docker cleanup.
...
The implemented CRM slice has no outstanding code-level blocker in this contract. Profile, tag, and pipeline-definition browser coverage becomes mandatory when a complete frontend consumer is introduced; until then those operations remain server-only or default-off.
