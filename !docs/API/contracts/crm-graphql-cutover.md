...
The generated consumer ledger and a source-scanning regression both prove that no shipped frontend called the aggregate REST route or currently calls the `contactProfile` GraphQL field. The unused Express aggregate and its stale child-query implementation are retired; `contactProfile` is the sole aggregate capability. The contact detail page intentionally composes narrower contact, activity, and related-content operations, so the aggregate remains server-only until a real product consumer is introduced and browser-tested.
