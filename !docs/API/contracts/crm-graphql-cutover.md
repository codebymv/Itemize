...
The NestJS `ContactsModule` now implements the read-only `contacts` and `contact` operations. PostgreSQL tests prove dual REST/GraphQL list membership, deterministic ordering, pagination totals, search/status/tag/assignee filters, detail projection, tenant-private missing-resource behavior, invalid-ID rejection, and suppression of user projections from corrupt cross-tenant references. The shared frontend contact API has a strict opt-in GraphQL transport that preserves the existing consumer shape and a REST-default rollback flag. Seven frontend tests cover dispatch, mapping, organization headers, error visibility, and nullable detail handling. A 2026-07-16 staging rehearsal passed authenticated browser list and detail reads through the legacy-origin `/graphql` proxy and passed the same journeys after rollback to REST. The flag remains disabled in deployed frontend builds and no production traffic uses these operations.
...
7. search/filter/pagination, login refresh, organization switching, mutations, and remaining contact/pipeline/form browser journeys pass semantic parity and rollback tests.
