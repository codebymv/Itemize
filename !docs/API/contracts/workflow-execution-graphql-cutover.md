...
**Status:** Operator GraphQL plus schedule/trigger workers implemented; enrollment/provider worker ownership remains
...
The legacy singleton remains unused, but first-party trigger callsites no longer depend on it. Contact creation/update, actual bulk tag addition/removal, deal-stage movement, form submission, booking create/cancel/reschedule, invoice-paid transitions, and linked-contact contract completion now commit a canonical `workflow_triggers` event in the same transaction as their domain mutation. Scheduled workflows persist one tenant-owned contact and one absolute run time. NestJS now owns explicit one-shot schedule and trigger runners: the dispatcher converts each due schedule into the shared queue exactly once, and the leased trigger worker matches active workflows, evaluates the retained trigger conditions, creates or reactivates enrollments, and completes the event transactionally. A separate enrollment scheduler still invokes the legacy step engine.

The NestJS commands `jobs:workflow-schedules` and `jobs:workflow-triggers` are bounded and inert unless explicitly invoked. They support optional ID-targeted execution for canaries, recover expired leases, fence stale attempts, redact stored failures, and preserve the retained retry/dead-letter schedule. No scheduler or deployment flag invokes them yet. The retained `WORKFLOW_ENROLLMENT_JOBS_ENABLED` gate continues to control due enrollment execution. This keeps operational ownership unchanged until the final configuration wave.

The retained minute scheduler still runs enabled workflow work through one ordered cycle: due one-shot schedules, trigger fan-out, enrollment execution, then provider side-effect delivery. It remains the runtime owner until the final scheduler reconciliation. The NestJS schedule and trigger entrypoints deliberately do not claim enrollment or provider work, so transferring either phase cannot silently broaden execution.
...
`WorkflowExecutionModule` now owns those GraphQL operations plus `retryWorkflowSideEffect`. `WorkflowJobsModule` owns due-schedule dispatch and trigger fan-out behind explicit one-shot entrypoints. Fresh PostgreSQL coverage proves exact status/type and due-work metrics, strict filters and bounded stable paging, safe error redaction, schema-level payload omission, tenant concealment, CSRF, dead-letter retry history, accepted-SID SMS reconciliation with correlated log persistence, single-winner schedule and trigger claims under contention, expired-lease recovery, stale-attempt fencing, and redacted trigger dead letters. The legacy HTTP adapters remain available for rollback while enrollment and provider workers are still transferred.
