...
- **Slice A shipped 2026-09-07.** Token blocks per hue and mode in `index.css` (`[data-theme-color="purple"]`, `.dark[data-theme-color="purple"]`, same for pink); `icon-accent` and `theme-tint` Tailwind colour keys; `lib/themeColor.ts` (palette, `applyThemeColor`, localStorage mirror `itemize:theme-color`) with the boot script in `index.html`; the interaction layer's hard-coded blue inks now read `--icon-accent`. Codemods: 218 icon-idiom sites in 82 files → `text-icon-accent`; 102 button-idiom strings in 70 files → `bg-primary text-primary-foreground`; 25 `#3B82F6` defaults in card, schema, category, calendar, and pipeline code → `DEFAULT_CARD_ACCENT` (`UI_COLORS.brandBlue` renamed `defaultCardAccent`). Brand allowlist pulled forward from Slice D as `design-system/brand-surfaces.ts` because the codemod and the ratchet both needed it. Ratchet contract landed in `visual-language.test.ts` at **520** raw blue classes on application surfaces; it fails in both directions, so every removal lowers the number in the same commit. Verified in the preview: stamping `data-theme-color` purple in light and pink in dark recolours buttons, chrome icons, and rings with no source change.
- **Slice B shipped 2026-09-07.** Migration 080 (`users.theme_color VARCHAR(16) NOT NULL DEFAULT 'blue'`, marker `user_theme_color_v1`); `CurrentUser.themeColor`; `updateViewerPreferences(input { themeColor })`, CSRF-protected on the AccountScoped auth resolver, validated against `backend/src/common/theme-color.ts` (no `mutationId`: setting a preference is idempotent, the same shape as `updateViewerProfile`). Frontend: `ThemeColorPicker` under Settings › Appearance (radio group, optimistic apply, revert + toast on failure), `useThemeColor` hook (attribute-backed), the auth context applies the account's theme on every hydration and clears it on logout, `updateCurrentUser` keeps the cached session in step; `ThemeFavicon` tints the shipped mark on a canvas for purple/pink (blue keeps the PNGs). `STATUS_THEME_CLASSES.blue` is now `.theme` (`bg-theme-tint` / `text-icon-accent` / `text-primary`), renamed across every `defineStatus` consumer; `defaultCardAccent()` reads the theme's 500 and `isDefaultAccent` accepts any theme default, so old blue cards keep blue while new cards start on the theme. Ratchet at **510**. Verified in the preview: pick purple → chrome, buttons, favicon, and the new-list default recolour; reload keeps it from the boot script; server reads back `purple`; `green` refused with BAD_USER_INPUT (integration spec).
- **Slice C shipped 2026-09-07.** A mapping codemod rewrote the remaining 510 raw blue classes on application surfaces (97 files): ink 500/600 and every dark-mode 300/400 → `text-icon-accent`, ink 700+ → `text-primary`; fills 400–600 → `bg-primary`, 700/800 → `bg-primary-hover`; washes 50–200 and 900/950 (alpha kept) → `bg-theme-tint`; borders → `border-primary` / `border-primary/30` (light) / `border-primary/40` (dark); rings → `ring-ring`; gradients → `from-primary` / `from-theme-tint`. A `dark:` twin that now maps to the same token as its light sibling was dropped. `primary`, `ring`, `icon-accent`, `theme-tint` carry `<alpha-value>` in Tailwind so `bg-primary/10` and `bg-theme-tint/30` really apply opacity. Inside cards: the whiteboard pen/eraser toggles and stroke slider and the wireframe arrow handles now wear the card accent (`accentFill`, `var(--card-accent)`), the same rule as the note toolbar. The admin email composer's in-editor prose styles (`[&_a]:…`, callouts, badges) followed the mapping; sent emails still render from backend brand constants. **Ratchet is at 0** — any new raw blue on an application surface fails the build.
- **Slice D shipped 2026-09-07.** `frontend/src/lib/brand.ts` (`BRAND_BLUE` #2563eb, `BRAND_BLUE_SOFT` #3B82F6, `BRAND_BLUE_DEEP` #1E40AF) and `backend/src/common/brand.ts` (the same plus `BRAND_BLUE_HOVER`, `BRAND_WASH`, `BRAND_TINT`, and `DEFAULT_CARD_ACCENT` for server-side card/category/tag/frame/calendar fallbacks). Every exported-document and email literal imports them: shared note/whiteboard/wireframe pages, whiteboard default ink and palette entry, landing-page document, chat and reputation widget defaults, invoice previews; transactional and campaign email, the unsubscribe page, admin email renderer, email-template prose styles, landing-page defaults. The admin email composer (`components/admin/RichTextEditor.tsx`) joined the brand allowlist and its in-editor prose styles went back to the brand blues so what the admin sees matches what the recipient gets. Values are unchanged everywhere, so no rendered output moved. Backend has no ESLint config on v9 (legacy `.eslintrc`); the backend gate is `tsc` + jest.

The programme is complete: brand surfaces read `BRAND_BLUE`, application surfaces read theme tokens with the ratchet at 0, and status colours stay in `lib/statusVisuals.ts`. Open decisions above stand as recommended (no neutral theme yet, cards start on the 500 step, organization-level defaults later).
