...
| Account and administration | Plan change, checkout/trial start, organization creation and switching, member invitations and resends, and Stripe connect/disconnect | Shared immediate locks close pre-render admission gaps; organization creation and invitation delivery retain stable keys for unchanged attempts; invitation rows use keyed locks so unrelated invitations remain actionable | Organization creation commits its owner membership, default selection, activity, and caller-scoped durable receipt atomically. Invitation create and resend commit organization-scoped receipts before delivery, pass the same attempt identity to the email provider, replay exact requests without duplicate delivery, reject changed or cross-action reuse, and fail closed when the original result is gone. | Confirmed organizations and invitations reconcile locally before narrow background refresh; authoritative entitlement or integration refresh confirms dependent state without converting a committed write into a false failure |
