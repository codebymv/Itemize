...
| Resource catalogs | Pipelines, products, pages, forms, calendars, automations, campaigns, segments, email templates, SMS templates, and reputation widgets | Resource-keyed immediate locks cover create, publish/status, duplicate, test, pause/resume, and delivery callbacks; unrelated rows remain concurrent. Pipeline, product, page, form, workflow, campaign, segment, email-template, SMS-template, and reputation-widget create/duplicate plus calendar create retain independent keys per unchanged attempt and per source row. | Pipeline, product, page, form, workflow, campaign, email-template, and SMS-template create/duplicate plus calendar, segment, and reputation-widget create commit an organization-scoped durable receipt, normalized intent fingerprint, and result identity in the insertion transaction. Exact retries replay the original resource, changed key reuse conflicts, and deleted results fail closed instead of silently creating a replacement. Pipeline creation serializes the default-pipeline invariant within the same boundary. Page, form, and workflow duplication share their resource quota boundary. Form and page publication are absolute status transitions and are safe to repeat. | Confirmed rows patch their owning catalog or preview state before narrowly invalidating derived lists |
