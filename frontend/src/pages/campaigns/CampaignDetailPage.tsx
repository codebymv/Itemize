...
import {
  ...
  type CampaignEditorBootstrapData,
} from "@/services/campaignEditorGraphql";
...
const toForm = (campaign: EmailCampaign): CampaignForm => ({
  ...
  segmentType:
    campaign.segment_type === "custom" ? "all" : campaign.segment_type,
  segmentId: campaign.segment_id || null,
  contactStatus:
    typeof campaign.segment_filter?.status === "string"
...
export function CampaignDetailPage() {
  ...
  const bootstrapQuery = useQuery({
    ...
    enabled: Boolean(organizationId) && !invalidCampaignId,
  ...
  const applyBootstrap = useCallback((bootstrap: CampaignEditorBootstrapData) => {
    setCampaign(bootstrap.campaign);
    if (bootstrap.campaign) {
      resetForm(bootstrap.campaign);
    } else {
      setForm(EMPTY_FORM);
    ...
    setAudiencePreview(bootstrap.audiencePreview);
  }, [resetForm]);

  useEffect(() => {
    if (!bootstrapQuery.data
      || initializedBootstrapRef.current === campaignBootstrapKey) return;
    applyBootstrap(bootstrapQuery.data);
    initializedBootstrapRef.current = campaignBootstrapKey;
  }, [applyBootstrap, bootstrapQuery.data, campaignBootstrapKey]);

  const editable =
    isNew || (campaign ? isCampaignEditable(campaign.status) : false);
  ...
  const recipientsQuery = useQuery({
    ...
    queryFn: ({ signal }) => getCampaignRecipients(
    ...
    enabled: Boolean(organizationId && campaign && !editable),
    refetchOnWindowFocus: false,
  });
  const recipients = recipientsQuery.data?.recipients ?? [];
  const recipientTotal = recipientsQuery.data?.pagination.total ?? 0;
  const recipientPages = recipientsQuery.data?.pagination.totalPages || 1;
  const recipientsLoading = recipientsQuery.isPending && recipientsQuery.fetchStatus !== "idle";
  const loading = orgLoading
    || (!invalidCampaignId && Boolean(organizationId) && bootstrapQuery.isPending)
    || (bootstrapQuery.isSuccess
      && initializedBootstrapRef.current !== campaignBootstrapKey);
  const loadError = invalidCampaignId
    ? "This campaign link is invalid."
    : bootstrapQuery.isError
      ? isNew
        ? "We could not prepare a new campaign. No campaign was created."
        : "We could not load this campaign. The campaign has not been changed."
      : null;

  const cacheCampaignBootstrap = (
    nextCampaign: EmailCampaign,
    ...
    queryClient.setQueryData<CampaignEditorBootstrapData>(targetKey, {
      ...
      segments,
      filterOptions,
      audiencePreview: nextAudiencePreview,
    });
  };

  const confirmLeave = (): boolean =>
    !dirty || window.confirm("Discard unsaved campaign changes?");
  const goBack = () => {
    if (confirmLeave()) navigate("/campaigns");
  };
  ...
  const previewHtml = editable
    ? form.contentSource === "template"
      ? getCampaignPreviewHtml(
          campaign || { content_html: null, template_html: null },
          selectedTemplate?.body_html,
        )
      : form.contentHtml
    : campaign
      ? getCampaignPreviewHtml(campaign)
      : "";
  ...
  const handleSave = async () => {
    ...
    if (!form.name.trim() || !form.subject.trim()) {
      toast({
        ...
        description: "Add a campaign name and email subject before saving.",
        variant: "destructive",
      });
      return;
    }
    if (form.contentSource === "template" && !form.templateId) {
      toast({
        ...
        description: "Select an email template or switch to custom content.",
        variant: "destructive",
      });
      return;
    }
    if (form.contentSource === "custom" && !form.contentHtml.trim()) {
      toast({
        ...
        description: "Add HTML content or select an email template.",
        variant: "destructive",
      });
      return;
    }
    if (form.segmentType === "segment" && !form.segmentId) {
      toast({
        ...
        description: "Select a saved segment for this audience.",
        variant: "destructive",
      });
      return;
    }
    if (form.segmentType === "tag" && form.tagIds.length === 0) {
      toast({
        title: "Tag required",
        description: "Select at least one included tag.",
        variant: "destructive",
      });
      return;
    }
    await runCampaignAction(async () => {
      try {
        ...
        content_text:
          form.contentSource === "custom" ? form.contentText || null : null,
        segment_type: form.segmentType,
        segment_id: form.segmentType === "segment" ? form.segmentId : null,
        segment_filter:
          form.segmentType === "status" ? { status: form.contactStatus } : {},
  ...
  const requireSaved = (): boolean => {
    ...
    toast({
      ...
      description: "Save the current setup before changing delivery state.",
      variant: "destructive",
    });
    return false;
  };
  ...
  const handleSchedule = async () => {
    ...
    if (!scheduleDate || !scheduleTime) {
      toast({
        ...
        description: "Choose a send date and time.",
        variant: "destructive",
      });
      return;
    }
    ...
    return (
      ...
      >
        <ErrorState
          ...
          description={loadError || "This campaign could not be found."}
          icon={Megaphone}
  ...
  const statusVisual = getCampaignStatusVisual(
    (campaign?.status || "draft") as Campaign["status"],
  );
  ...
  const primaryAction = editable ? (
    <HeaderAction
      label={working ? "Saving..." : isNew ? "Create campaign" : "Save changes"}
  ...
  const filteredRecipients = recipients.filter((recipient) => {
    ...
    return (
      ...
      recipientDisplayName(recipient).toLowerCase().includes(query)
    );
  });
    ...
    >
      <EntityDetailHeader
        ...
        descriptor={
          campaign?.subject ||
          form.subject ||
          "Build the message, audience, and delivery setup"
        }
        metadata={
          <>
            {campaign ? (
            ...
            {campaign?.created_by_name && (
              <span>by {campaign.created_by_name}</span>
            )}
            {campaign?.scheduled_at && (
              <span>Scheduled {formatDateTime(campaign.scheduled_at)}</span>
            )}
            {campaign?.completed_at && campaign.status === "sent" && (
              <span>Delivered {formatDateTime(campaign.completed_at)}</span>
            )}
          </>
        }
      />
      ...
      {editable ? (
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateForm(
                        "contentSource",
                        value as CampaignForm["contentSource"],
                      )
                    }
                  >
                    <SelectTrigger aria-label="Content source">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                {form.contentSource === "template" ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex min-w-0 items-center gap-3 rounded-lg border p-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-muted-foreground">
                          {selectedTemplate?.subject ||
                            "Choose from your reusable email templates."}
                        </p>
                      </div>
                      <Button
                        type="button"
                      ...
                      >
                        {selectedTemplate ? "Change" : "Browse templates"}
                      </Button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(event) =>
                          updateForm("contentHtml", event.target.value)
                        }
                        className="min-h-44 font-mono text-sm"
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="campaign-text">Plain-text fallback</Label>
                      <Textarea
                        id="campaign-text"
                        value={form.contentText}
                        onChange={(event) =>
                          updateForm("contentText", event.target.value)
                        }
                        className="min-h-24"
                      />
                    </div>
              ...
              >
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateForm(
                        "segmentType",
                        value as CampaignForm["segmentType"],
                      )
                    }
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="tag">Contact tags</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                {form.segmentType === "segment" && (
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateForm("segmentId", Number(value))
                      }
                    >
                ...
                {form.segmentType === "status" && (
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger aria-label="Contact status">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2 rounded-md border p-3">
                      {filterOptions.tags.map((tag) => (
                        ...
                        >
                          <Checkbox
                            checked={form.excludedTagIds.includes(tag.id)}
                            onCheckedChange={() =>
                              toggleTag("excludedTagIds", tag.id)
                            }
                          />
                          <span>{tag.name}</span>
                        </label>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium">Eligible audience</p>
              ...
              >
                {campaign ? (
                  <>
                    ...
                    {dirty && (
                      <p className="text-xs text-muted-foreground">
                        Save campaign changes before scheduling or sending.
                      </p>
                    )}
                  </>
                ) : (
                  <div className="rounded-lg border border-dashed p-4">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Scheduling and sending become available after this
                      campaign has a saved audience and message.
                    </p>
                  </div>
                )}
              </FramedSection>
          ...
          >
            {previewHtml ? (
              ...
              <div className="flex h-64 items-center justify-center rounded-lg border text-sm text-muted-foreground">
                Add campaign content to see its preview.
              </div>
            )}
          </FramedSection>
        </div>
        ...
        <div className="space-y-6">
          <FramedSection title="Performance" icon={BarChart3}>
            <ResponsiveCardRail
            ...
            >
              <StatCard
                title="Bounced campaign messages"
                badgeText="Bounced"
                value={formatInteger(campaign.total_bounced)}
                icon={XCircle}
                description={`${campaign.bounce_rate}% of sent`}
                colorTheme="red"
              />
              <StatCard
                title="Campaign recipients"
                badgeText="Recipients"
                value={formatInteger(campaign.total_recipients)}
                icon={Users}
                description={`${formatInteger(campaign.total_sent)} sent`}
                colorTheme="blue"
              />
              <StatCard
                title="Delivered campaign messages"
                badgeText="Delivered"
                value={formatInteger(campaign.total_delivered)}
                icon={CheckCircle}
                description={`${percentOf(campaign.total_delivered, campaign.total_recipients).toFixed(1)}% of recipients`}
                colorTheme="green"
              />
              <StatCard
                title="Opened campaign messages"
                badgeText="Opened"
                value={formatInteger(campaign.total_opened)}
                icon={Eye}
                description={`${campaign.open_rate}% opened`}
                colorTheme="green"
              />
              <StatCard
                title="Clicked campaign messages"
                badgeText="Clicked"
                value={formatInteger(campaign.total_clicked)}
                icon={MousePointerClick}
                description={`${campaign.click_rate}% clicked`}
                colorTheme="green"
              />
            </ResponsiveCardRail>
          </FramedSection>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <FramedSection
              ...
              contentClassName="flex flex-1 flex-col justify-between gap-5"
            >
              {[
                {
                  label: "Sent",
                ...
                <div key={item.label} className="space-y-2">
                  <div className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="font-medium">
                      ...
                      {percentOf(item.value, campaign.total_recipients).toFixed(
                        1,
                      )}
                      %
                    </span>
                  ...
                  <Progress
                    value={percentOf(item.value, campaign.total_recipients)}
                    className="h-2.5"
            ...
            >
              <div>
                ...
                <p className="mt-1 font-medium capitalize">{audienceLabel}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Email template</p>
                <p className="mt-1 font-medium">
                  {campaign.template_name || "Custom content"}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">From</p>
                <p className="mt-1 font-medium">
                  {campaign.from_name || "Not set"}
              ...
              <div>
                ...
                <p className="mt-1 font-medium">
                  {campaign.reply_to || campaign.from_email || "Not set"}
                </p>
              </div>
              <div>
            ...
            action={
              <PageToolbar
                ...
                filters={
                  ...
                  >
                    <SelectTrigger aria-label="Recipient delivery status" className="w-[8.5rem]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      {RECIPIENT_STATUSES.map((status) => (
                        <SelectItem key={status} value={status}>
                          {RECIPIENT_VISUALS[status].label}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                meta={
                  <span className="whitespace-nowrap text-sm text-muted-foreground">
                    {recipientTotal} recipient{recipientTotal === 1 ? "" : "s"}
                  </span>
                }
              />
            }
          >
            {recipientsQuery.isError ? (
              ...
              <EmptyState
                ...
                description="Delivery records will appear as the campaign processes recipients."
                className="py-10"
              />
            ) : (
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredRecipients.map((recipient) => {
                      const visual = RECIPIENT_VISUALS[recipient.status];
                      const openContact = () =>
                        navigate(`/contacts/${recipient.contact_id}`);
                      return (
                        <TableRow
                          key={recipient.id}
                          interactive
            ...
            {recipientPages > 1 && (
              <div className="flex items-center justify-between border-t p-4">
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setRecipientPage((page) => Math.max(1, page - 1))
                  }
                ...
                <Button
                  ...
                  onClick={() =>
                    setRecipientPage((page) =>
                      Math.min(recipientPages, page + 1),
                    )
                  }
                  disabled={
                    recipientPage >= recipientPages || recipientsLoading
                  }
                >
                  Next
                </Button>
                  ...
                  <TableBody>
                    {campaign.links.map((link) => (
                      <TableRow key={link.id}>
                        <TableCell>
                          <p className="font-medium">
                            {link.link_text || link.original_url}
                          </p>
          ...
          >
            {previewHtml ? (
              ...
              <PreviewPlaceholder
                ...
                description="Add email content to generate this snapshot."
                className="h-64"
              />
            )}
          </FramedSection>
        ...
        if (!open) resetCampaignSendAttempt();
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {audiencePreview
                  ? `This will start delivery to ${audiencePreview.recipientCount} eligible recipient${audiencePreview.recipientCount === 1 ? "" : "s"}. Delivery cannot be undone after it begins.`
      ...
      <Dialog open={testOpen} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Send the current saved campaign content to one address. This does
              not enroll a recipient or change campaign statistics.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="campaign-test-email">Destination email</Label>
            <Input
