...
import {
  ...
  REPUTATION_PLATFORM_LABELS,
  type ReputationPlatformKey,
} from "./constants/reputationVisuals";

type WidgetDraft = Omit<
  ReviewWidget,
  "id" | "organization_id" | "widget_key" | "created_at" | "updated_at"
>;
...
const WIDGET_TYPES: Array<ReviewWidget["widget_type"]> = [
  "carousel",
  "grid",
  "list",
  "badge",
...
export function ReputationWidgetEditorPage() {
  ...
  const { organizationId, isLoading: organizationLoading, error: initError } = useOrganization({
  ...
  const widgetQuery = useQuery({
    ...
    queryFn: ({ signal }) => getWidgetViaGraphql(
      widgetId as number,
      organizationId as number,
      signal,
    ),
    enabled: !isNew && validWidgetId && organizationId !== null,
    staleTime: QUERY_STALE_TIME_MS,
    retry: shouldRetryQuery,
  });
  const previewReviewsQuery = useQuery({
    queryKey: ["reputation-widget-preview-reviews", organizationId],
  ...
  const embedCodeQuery = useQuery({
    queryKey: ["reputation-widget-embed-code", organizationId, widget?.id],
    queryFn: ({ signal }) => getWidgetEmbedCodeViaGraphql(
      widget?.id as number,
      organizationId as number,
      signal,
    ),
    enabled: mode === "install" && organizationId !== null && Boolean(widget?.id),
    staleTime: QUERY_STALE_TIME_MS,
    retry: shouldRetryQuery,
  });
  const reviews = previewReviewsQuery.data?.reviews ?? [];
  const embedCode = embedCodeQuery.data?.embed_code ?? "";
  const loading = organizationLoading || (!isNew && validWidgetId && widgetQuery.isPending);
  const loadError = !isNew && !validWidgetId
    ? "Invalid widget ID."
    : widgetQuery.error && !widgetQuery.data
      ? widgetQuery.error instanceof Error ? widgetQuery.error.message : "Could not load this widget."
      : null;
  const { isDirty, markClean } = useDirtyState({
    value: draft,
    ready: !loading,
    resetKey: widget?.id ?? "new",
  });
  const { confirmLeave } = useUnsavedChangesGuard({
    when: isDirty || saving,
    message: "This widget has unsaved changes. Leave without saving them?",
  });
  ...
  useEffect(() => {
    ...
    const selected = widgetQuery.data;
    if (!selected || widget?.id === selected.id) return;
    const {
      id: _id,
    ...
    setWidget(selected);
    setDraft(values);
  }, [isNew, widget?.id, widgetQuery.data]);

  const updateDraft = <K extends keyof WidgetDraft>(
    field: K,
    value: WidgetDraft[K],
  ) => {
  ...
  const save = async () => {
    ...
    if (
      ![draft.primary_color, draft.background_color, draft.text_color].every(
        isHexColor,
      )
    ) {
      toast({
        title: "Use six-digit hex colors",
        description: "For example, #2563EB.",
        variant: "destructive",
      });
      return;
    }
    await runSave(async () => {
      ...
      queryClient.setQueryData(
        ["reputation-widget", organizationId, saved.id],
        saved,
      );
      queryClient.setQueryData<ReviewWidget[]>(
        ["reputation-widgets", organizationId],
        current => current
      ...
      queryClient.removeQueries({
        queryKey: ["reputation-widget-embed-code", organizationId, saved.id],
      });
  ...
  const statusVisual = getReviewWidgetAvailabilityVisual(
    widget?.is_active ?? draft.is_active,
  );
  const StatusIcon = statusVisual.icon;
  const saveDisabled = saving || (!isNew && !isDirty);
  const backButton = (
    <ShellBackButton
      label="Back to widgets"
      onClick={() => {
        if (confirmLeave()) navigate("/review-widgets");
      }}
    />
  );
      ...
      >
        {initError ? (
          <OrganizationErrorState
            title="Unable to load review widget"
            icon={LayoutGrid}
          />
        ) : (
          <ErrorState
            kind="page"
            title="Review widget unavailable"
            description={loadError || undefined}
            onAction={() => void widgetQuery.refetch()}
          />
        )}
      </PageLayout>
    );
    ...
    >
      <EntityDetailHeader
        ...
        metadata={
          widget ? (
            <>
              <span>{draft.min_rating}+ stars</span>
              <span>Up to {draft.max_reviews} reviews</span>
            </>
          ) : undefined
        }
      />
            ...
            <TabsContent value="settings" className="mt-0 space-y-6">
              <Card>
                ...
                <CardContent surface="inset" className="space-y-5">
                  <AvailabilitySettingRow
                    id="review-widget-active"
                    label="Available on installed sites"
                    checked={draft.is_active}
                    onCheckedChange={(value) => updateDraft("is_active", value)}
                    help="Unavailable widgets retain their settings but do not render on installed sites."
                    helpLabel="About widget availability"
                  />
                  <div className="grid gap-4 border-t pt-5 sm:grid-cols-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateDraft(
                            "widget_type",
                            value as ReviewWidget["widget_type"],
                          )
                        }
                      >
                        ...
                        <SelectContent>
                          {WIDGET_TYPES.map((type) => (
                            <SelectItem key={type} value={type}>
                              {getReputationPlatformLabel(type)}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="widget-min-rating">Minimum rating</Label>
                      <Select
                        value={String(draft.min_rating)}
                        onValueChange={(value) =>
                          updateDraft("min_rating", Number(value))
                        }
                      >
                        <SelectTrigger id="widget-min-rating">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="widget-max-reviews">
                        Maximum reviews
                      </Label>
                      <Input
                        id="widget-max-reviews"
                        type="number"
                        min={1}
                        max={100}
                        value={draft.max_reviews}
                        onChange={(event) =>
                          updateDraft(
                            "max_reviews",
                            Math.max(
              ...
              <Card>
                ...
                <CardContent surface="inset" className="space-y-5">
                  <div>
                    <SettingsFieldLabel
                      help="Leave every option unchecked to include reviews from all sources."
                      helpLabel="About platform selection"
                    >
                      Platforms
                    </SettingsFieldLabel>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-sm">
                              {REPUTATION_PLATFORM_LABELS[platform]}
                            </span>
                          </label>
                        );
                      })}
                  ...
                  <div className="grid divide-y rounded-lg border sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                    <div className="flex items-center justify-between gap-3 p-3">
                      <Label htmlFor="widget-hide-empty">
                        Require review text
                      </Label>
                      <Switch
                        id="widget-hide-empty"
                        checked={draft.hide_no_text_reviews}
                        onCheckedChange={(value) =>
                          updateDraft("hide_no_text_reviews", value)
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between gap-3 p-3">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(value) =>
                          updateDraft("auto_refresh", value)
                        }
                      />
                    </div>
                  </div>
                  {draft.auto_refresh ? (
                    <div className="grid gap-2 sm:max-w-xs">
                      <Label htmlFor="widget-refresh-hours">
                        Refresh interval (hours)
                      </Label>
                      <Input
                        id="widget-refresh-hours"
                        type="number"
                        min={1}
                        max={168}
                        value={draft.refresh_interval_hours}
                        onChange={(event) =>
                          updateDraft(
                            "refresh_interval_hours",
                            Math.max(
                              1,
                              Math.min(168, Number(event.target.value) || 1),
                            ),
                          )
                        }
                      />
              ...
              <Card>
                ...
                <CardContent surface="inset" className="space-y-5">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="grid gap-2">
                      ...
                      <Select
                        value={draft.theme}
                        onValueChange={(value) =>
                          updateDraft("theme", value as ReviewWidget["theme"])
                        }
                      >
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          updateDraft(
                            ...
                            Math.max(
                              0,
                              Math.min(32, Number(event.target.value) || 0),
                            ),
                          )
                        }
                      />
                    ...
                    {(
                      ...
                      <div key={field} className="grid gap-2">
                        <Label htmlFor={`widget-${field}`}>{label}</Label>
                  ...
                  <div className="grid divide-y rounded-lg border sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                    {(
                      [
                        ["show_rating_stars", "Rating stars"],
                        ["show_reviewer_photo", "Reviewer photo"],
                        ["show_review_date", "Review date"],
                        ["show_platform_icon", "Platform icon"],
                      ] as const
                    ).map(([field, label]) => (
                      <div
                        key={field}
                      ...
                      >
                        ...
                        <Switch
                          id={`widget-${field}`}
                          checked={draft[field]}
                          onCheckedChange={(value) => updateDraft(field, value)}
                        />
                      </div>
                    ))}
                  </div>
            ...
            <TabsContent value="install" className="mt-0">
              <Card>
                ...
                <CardContent surface="inset">
                  {embedCodeQuery.isPending && widget ? (
                    <Skeleton className="h-32" />
                  ) : embedCodeQuery.error && widget ? (
                    <ErrorState
                      kind="section"
                      icon={Code2}
                      title="Unable to load install code"
                      onRetry={() => void embedCodeQuery.refetch()}
                    />
                  ) : embedCode ? (
