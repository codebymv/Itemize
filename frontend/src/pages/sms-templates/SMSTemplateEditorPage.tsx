...
export function SMSTemplateEditorPage() {
  ...
  const loadTemplate = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(
        "We could not load this SMS template. No changes were made.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateMessageInfo = useMemo(
    () =>
      debounce(async (message: string) => {
        if (!message) return setMessageInfo(EMPTY_INFO);
        try {
          setMessageInfo(await getMessageInfo(message));
        } catch {
          setMessageInfo(localMessageInfo(message));
        }
      }, 300),
    [],
  );
      ...
      >
        {orgError ? (
          <OrganizationErrorState
            title="Unable to load SMS template"
            icon={MessageSquare}
          />
        ) : (
          <ErrorState
            kind="page"
            title="SMS template unavailable"
  ...
  return (
    ...
    >
      <EntityDetailHeader
        ...
        descriptor={
          state.message ? (
            <p className="max-w-xl truncate">{state.message}</p>
          ) : undefined
        }
        metadata={
          <>
            <span>
              {state.category ? categoryLabel(state.category) : "Uncategorized"}
            </span>
            <span>{messageInfo.length} characters</span>
            <span>
              {messageInfo.segments} segment
              {messageInfo.segments === 1 ? "" : "s"}
            </span>
          </>
        }
      />
      ...
      <div className="grid items-start gap-6 xl:grid-cols-3">
        <div className="space-y-6 xl:col-span-2">
          <Card>
            ...
            <CardContent surface="inset" className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger id="sms-template-category">
              ...
              <AvailabilitySettingRow
                ...
                label="Available for campaigns and automations"
          ...
          <Card>
            ...
            <CardContent surface="inset" className="space-y-3">
              <Label htmlFor="sms-template-message" className="sr-only">
                Message
              </Label>
              <Textarea
                ref={messageRef}
        ...
        <Card className="xl:sticky xl:top-6">
          ...
          <CardContent surface="inset">
            <div className="rounded-[2rem] border bg-muted/20 p-4">
