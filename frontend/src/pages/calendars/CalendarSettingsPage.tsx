...
const comparableWindows = (windows: AvailabilityWindow[]) =>
...
export function CalendarSettingsPage() {
  ...
  const settingsDirty = useMemo(
    () =>
      Boolean(
        calendar &&
        draft &&
        JSON.stringify(draft) !== JSON.stringify(makeDraft(calendar)),
      ),
    [calendar, draft],
  );
  const availabilityDirty = useMemo(
    () =>
      Boolean(
        calendar &&
        JSON.stringify(comparableWindows(availability)) !==
          JSON.stringify(
  ...
  const changeWindow = (index: number, update: Partial<AvailabilityWindow>) => {
  ...
  const saveAvailability = async () => {
    ...
    const invalidWindow = availability.some(
  ...
  const saveOverride = async () => {
    ...
    if (
      ...
      overrideDraft.startTime >= overrideDraft.endTime
    ) {
      toast({
        title: "Check override times",
    ...
    try {
      const created = await addDateOverride(
        calendar.id,
        {
          override_date: overrideDraft.date,
          is_available: overrideDraft.isAvailable,
          start_time: overrideDraft.isAvailable
            ? overrideDraft.startTime
            : undefined,
          end_time: overrideDraft.isAvailable
            ? overrideDraft.endTime
            : undefined,
      ...
      setCalendar((previous) => {
        ...
        const retained = (previous.date_overrides ?? []).filter(
          (item) => item.override_date !== created.override_date,
        );
        return {
          ...previous,
          date_overrides: [...retained, created].sort((left, right) =>
            left.override_date.localeCompare(right.override_date),
          ),
        };
      });
      setOverrideDraft({
      ...
      toast({
        title: "Unable to save date override",
        description: apiErrorMessage(error, "Try again."),
        variant: "destructive",
      });
    } finally {
    ...
    try {
      ...
      setCalendar((previous) =>
        previous
          ? {
              ...previous,
              date_overrides: (previous.date_overrides ?? []).filter(
      ...
      toast({
        title: "Unable to remove date override",
        description: apiErrorMessage(error, "Try again."),
        variant: "destructive",
      });
    }
    ...
    <PageLayout
      ...
      headerTools={{
        ...
        primaryAction: (
          <HeaderAction
            label={
              savingSettings || savingAvailability
                ? "Saving..."
                : "Save changes"
            }
            icon={
            ...
            disabled={
              savingSettings ||
              savingAvailability ||
              (!settingsDirty && !availabilityDirty)
            }
            busy={savingSettings || savingAvailability}
          />
        ),
      }}
    >
      <div className="space-y-6">
        <EntityDetailHeader
          ...
          icon={
            <CalendarDays
              className={cn("h-6 w-6", persistedVisual.iconClass)}
            />
          }
        ...
        <div className="grid items-start gap-6 2xl:grid-cols-[minmax(0,1fr)_minmax(27rem,0.72fr)]">
          <div className="order-2 space-y-6 2xl:order-1">
              ...
              <CardContent surface="inset">
                ...
                >
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="calendar-duration">
                        Duration (minutes)
                      </Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="calendar-buffer-before">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="calendar-buffer-after">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="calendar-min-notice">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="calendar-max-future">
                        Booking window (days)
                      </Label>
                      <Input
            ...
            <Card>
              <CardHeader>
                <SectionCardTitle icon={CalendarClock}>
                  Weekly availability
                </SectionCardTitle>
              </CardHeader>
              <CardContent surface="inset" className="divide-y">
                        ...
                        {windows.map(({ window, index }, windowIndex) => (
                          ...
                          >
                            <Input
                              ...
                              onInput={(event) =>
                                changeWindow(index, {
                                  start_time: (event.target as HTMLInputElement)
                                    .value,
                                })
                              }
                            />
                            ...
                            <Input
                              ...
                              onInput={(event) =>
                                changeWindow(index, {
                                  end_time: (event.target as HTMLInputElement)
                                    .value,
                                })
                              }
                            />
            ...
            <Card>
              <CardHeader>
                <SectionCardTitle icon={CalendarOff}>
                  Date overrides
                </SectionCardTitle>
              </CardHeader>
                ...
                <div className="space-y-4 rounded-lg border bg-muted/15 p-4">
                  <div
                    className={cn(
                      ...
                      overrideDraft.isAvailable &&
                        "lg:grid-cols-[minmax(12rem,1fr)_minmax(9rem,0.7fr)_minmax(16rem,1.25fr)]",
                    )}
                  >
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex h-11 items-center justify-between gap-3 rounded-md border bg-background px-3">
                        ...
                        <Switch
                          ...
                          aria-label="Make this date available"
                  ...
                  <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                    <div className="min-w-0 space-y-2">
                      ...
                      <Input
                        id="override-reason"
                        placeholder="Holiday, extended hours…"
                    ...
                    <Button
                      ...
                      disabled={!overrideDraft.date || savingOverride}
                    ...
                    {calendar.date_overrides?.map((override) => (
                      ...
                      >
                        ...
                        <Button
                          ...
                          size="icon"
                          className="shrink-0"
                          aria-label={`Remove override for ${override.override_date}`}
