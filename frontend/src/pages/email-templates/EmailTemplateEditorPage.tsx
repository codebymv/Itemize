...
const EMPTY_STATE: EditorState = {
  name: '', category: 'general', isActive: true,
  subject: '', preheader: '', bodyHtml: '<p></p>', bodyText: '',
};

const TEMPLATE_CATEGORIES = ['general', 'marketing', 'sales', 'onboarding', 'transactional'] as const;
const normalizeCategory = (category?: string | null): string => category?.trim().toLowerCase() || 'general';
const categoryLabel = (category: string): string => category.charAt(0).toUpperCase() + category.slice(1);
...
const messageSummary = (html: string) => html.replace(/<[^>]*>/g, ' ').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
...
export function EmailTemplateEditorPage() {
  ...
  useEffect(() => { setStudioOpen(true); setStudioMode('edit'); }, [id]);
  useEffect(() => {
    if (!templateBrowserOpen || !organizationId) return;
    let active = true;
    setTemplateLibraryLoading(true);
    getEmailTemplates(organizationId)
      .then(response => { if (active) setTemplateLibrary(response.templates || []); })
      .catch(() => {
        if (active) toast({ title: 'Unable to load templates', description: 'Your template library is temporarily unavailable.', variant: 'destructive' });
      })
  ...
  const persistDraft = useCallback(async (
    ...
    try {
      ...
      if (options.notify !== false) {
        toast({ title: 'Draft saved', description: 'The published version remains live until you publish.' });
      }
      return saved;
    } catch {
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      let target = template;
      if (!target || isDirty) target = await persistDraft({ navigateAfterCreate: false, notify: false });
      if (!target) return;
      ...
      toast({ title: 'Template published', description: 'New sends will use this version.' });
    } catch {
  ...
  const handleTest = useCallback(async () => {
    if (!organizationId || !currentUser?.email) {
      toast({ title: 'Test email unavailable', description: 'Your account needs a delivery email address.', variant: 'destructive' });
      return;
    }
    let target = template;
    if (!target || isDirty) target = await persistDraft({ navigateAfterCreate: false, notify: false });
    ...
    setTesting(true);
    try {
      const useDraft = Boolean(target.has_unpublished_changes || !target.published_version);
      await sendTestEmail(target.id, organizationId, currentUser.email, undefined, useDraft);
      toast({ title: 'Test email queued', description: `Sending this ${useDraft ? 'draft' : 'published version'} to ${currentUser.email}.` });
    } catch {
      toast({ title: 'Unable to send test', description: 'Your template has not been changed.', variant: 'destructive' });
    } finally {
      setTesting(false);
    }
  ...
  const goBack = () => { if (confirmLeave()) navigate('/email-templates'); };
  const leading = <ShellBackButton label="Back to email templates" onClick={goBack} />;
  ...
  const studioEditor = (
    <Card>
      <CardContent className="pt-6"><EmailContentEditor header={<SectionCardTitle icon={Mail}>Email content</SectionCardTitle>} value={state} onChange={content => setState(current => ({ ...current, ...content }))} disabled={busy} /></CardContent>
    </Card>
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid items-stretch gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <div className="flex items-center justify-between gap-4"><span className="text-muted-foreground">Category</span><span>{categoryLabel(state.category)}</span></div>
            <div className="flex items-center justify-between gap-4"><span className="text-muted-foreground">Availability</span><span>{state.isActive ? 'Available' : 'Inactive'}</span></div>
            <div className="flex items-center justify-between gap-4"><span className="text-muted-foreground">Live version</span><span>{template?.published_version ? `Version ${template.published_version}` : 'Not published'}</span></div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><SectionCardTitle icon={Mail}>Email content</SectionCardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div><p className="text-xs text-muted-foreground">Subject</p><p className="mt-1 text-sm">{state.subject || 'Not set'}</p></div>
            {state.preheader && <div><p className="text-xs text-muted-foreground">Preview text</p><p className="mt-1 text-sm">{state.preheader}</p></div>}
            <p className="line-clamp-2 text-sm text-muted-foreground">{messageSummary(state.bodyHtml) || 'No message yet'}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <EmailStudioDialog
        ...
        editor={studioEditor}
        headerActions={(
          <Button type="button" variant="outline" size="sm" className="h-8 px-2 text-blue-600 hover:bg-blue-500/10 hover:text-blue-700 dark:text-blue-400 dark:hover:bg-blue-400/10 dark:hover:text-blue-300 sm:px-3" onClick={() => setTemplateBrowserOpen(true)}>
            <FileText className="h-4 w-4" /><span className="hidden sm:inline">Templates</span><span className="sr-only sm:hidden">Browse templates</span>
          </Button>
        )}
        mode={studioMode}
      ...
      <EmailTemplateBrowserDialog
        ...
        headerAction={(
          <Button type="button" variant="outline" size="sm" className="h-8 px-2 text-blue-600 hover:bg-blue-500/10 hover:text-blue-700 dark:text-blue-400 dark:hover:bg-blue-400/10 dark:hover:text-blue-300 sm:px-3" onClick={() => setTemplateDetailsOpen(true)}>
            <Settings2 className="h-4 w-4" /><span className="hidden sm:inline">Template details</span><span className="sr-only sm:hidden">Template details</span>
          </Button>
        )}
        onSelect={item => {
        ...
        renderPreview={item => <EmailPreviewPane organizationId={organizationId} content={{ subject: item.draft_subject ?? item.subject, preheader: item.draft_preheader ?? item.preheader ?? '', bodyHtml: item.draft_body_html ?? item.body_html, bodyText: item.draft_body_text ?? item.body_text ?? '' }} className="h-full" />}
      ...
      <Dialog open={templateDetailsOpen} onOpenChange={setTemplateDetailsOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">Manage this template's name, category, and availability.</DialogDescription>
          </DialogHeader>
