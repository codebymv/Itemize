...
vi.mock('@/services/gleamPairingGraphql', () => ({
  createGleamPairing: vi.fn(),
  changeGleamPairing: vi.fn(),
  getGleamPairing: vi.fn(),
}));
const initial = {
  enabled: true,
  organizationId: 1,
...
it('freezes the code, assignee and request key across uncertain creation', async () => {
  ...
  expect(vi.mocked(createGleamPairing).mock.calls[1]).toEqual(
    vi.mocked(createGleamPairing).mock.calls[0],
  );
  expect(vi.mocked(createGleamPairing).mock.calls[0][1].code).toMatch(
    /^[a-f0-9]{64}$/,
  );
});
it('requires explicit approval of the verified source organization and shows the saved assignee', async () => {
  ...
  vi.mocked(changeGleamPairing).mockResolvedValue({
    ...pending,
    pairing: { ...pending.pairing, state: 'approved' },
  });
  render(<GleamIntegration organizationId={1} />);
  await screen.findByText(/New tasks will be assigned to Sam/);
  expect(changeGleamPairing).not.toHaveBeenCalled();
  fireEvent.click(
    screen.getByRole('button', { name: 'Approve connection to Gleam A' }),
  );
  await screen.findByText(/Approved for Gleam A/);
  expect(changeGleamPairing).toHaveBeenCalledWith(
    1,
    'approveGleamPairing',
    'request-id',
    expect.any(String),
...
it('shows the active delivery state and saved task defaults', async () => {
  ...
  expect(
    screen.getByRole('button', { name: 'Disconnect in Itemize' }),
  ).toBeInTheDocument();
});
