...
export class SignatureTemplatesRepository {
  ...
  async delete(organizationId:number,id:number):Promise<SignatureTemplateRow|null>{return this.transaction(async client=>{const row=await this.selectTemplateOrNull(client,organizationId,id,true);if(!row)return null;await client.query('DELETE FROM signature_templates WHERE id=$1 AND organization_id=$2',[id,organizationId]);return row;});}
