/**
 * Faithful port of the retained public-form validation contract
 * (backend/src/utils/publicFormContract.js). Error messages, codes, and
 * field attribution are part of the retained HTTP protocol and must not
 * drift from the legacy implementation while both runtimes serve it.
 */
import validator from 'validator';

export const MAX_PUBLIC_FORM_BYTES = 64 * 1024;
export const MAX_PUBLIC_FORM_FIELDS = 100;
...
export class PublicFormValidationError extends Error {
  ...
  readonly fieldId: number | string | null;
...
const normalizedOptions = (field: PublicFormField): string[] => {
...
export function validateFormDefinition(fields: PublicFormField[]): true {
  if (
    !Array.isArray(fields) ||
    fields.length === 0 ||
    fields.length > MAX_PUBLIC_FORM_FIELDS
  ) {
    throw new PublicFormValidationError(
  ...
  for (const field of fields) {
    ...
    if (
      ...
      !CONTACT_FIELDS.has(field.map_to_contact_field)
    ) {
      throw new PublicFormValidationError(
    ...
    for (const condition of conditions as FieldCondition[]) {
      ...
      if (
        ...
        !CONDITION_OPERATORS.has(operator as string) ||
...
const fieldState = (
  ...
  const conditions = (field.conditions || []) as FieldCondition[];
  const showConditions = conditions.filter(
    (condition) => (condition.action || 'show') === 'show',
  );
  ...
  const active =
    showConditions.every((condition) => conditionMatches(condition, data)) &&
...
const normalizeString = (
  ...
  if (
    normalized.length < Number(minLength) ||
    normalized.length > Number(maxLength)
  ) {
    throw new PublicFormValidationError(
...
export function validatePublicFormSubmission(
  ...
  if (unknownId) {
    throw new PublicFormValidationError(
      'Form data contains an unknown field',
      unknownId,
    );
  }
