...
The five authenticated reputation-review CRUD operations are assigned to `ReputationReviewsModule`. Stable bounded reads, tenant-qualified joins and references, manual sentiment, row-locked response-state updates, CSRF, and rollback are frozen in the [Reputation reviews GraphQL cutover contract](contracts/reputation-reviews-graphql-cutover.md). Reputation platforms, requests, widgets, settings, analytics, and public collection remain separate slices.

All 9 dashboard and analytics operations are assigned to `AnalyticsModule`. Strict period/ID inputs, tenant and numeric normalization, bucket merging, selected-pipeline funnels, communication lifecycle counts, and the unresolved revenue/lifecycle/timezone/stage-history decisions are frozen in the [Analytics and dashboard GraphQL cutover contract](contracts/analytics-graphql-cutover.md). `dashboardAnalytics`, contact trends, deal performance, booking summary, communication stats, and workflow performance are implemented behind six independent default-off frontend flags. Conversion rates, revenue trends, and pipeline velocity remain blocked on explicit domain decisions rather than being mechanically copied from misleading legacy semantics.
