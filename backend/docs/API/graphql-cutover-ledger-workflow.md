...
All 9 core dashboard and Analytics operations plus the reputation aggregate belong to `AnalyticsModule`. Strict period/ID inputs, tenant and numeric normalization, selected-pipeline funnels, communication lifecycle counts, and the corrected conversion, revenue, and open-deal-age definitions are frozen in the [Analytics and dashboard GraphQL cutover contract](contracts/analytics-graphql-cutover.md). The dashboard calls GraphQL directly for all nine reads with no Analytics HTTP fallback: conversion excludes the nonexistent lead/customer lifecycle, revenue separates booked and collected values by currency, and `pipelineDealAge` does not claim unavailable stage-transition velocity. Reputation analytics is independently complete.
