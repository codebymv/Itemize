...
The five authenticated reputation-review CRUD operations are assigned to `ReputationReviewsModule`. Stable bounded reads, tenant-qualified joins and references, manual sentiment, row-locked response-state updates, CSRF, and rollback are frozen in the [Reputation reviews GraphQL cutover contract](contracts/reputation-reviews-graphql-cutover.md). The Reviews-page aggregate is independently assigned to `AnalyticsModule` with one tenant snapshot and a parameterized period in the [Reputation analytics GraphQL cutover contract](contracts/reputation-analytics-graphql-cutover.md). Review-request list/delete management is assigned to `ReputationRequestsModule` in the [Reputation request management GraphQL cutover contract](contracts/reputation-requests-graphql-cutover.md); provider-facing send, bulk-send, and resend remain separate durable-delivery work. Reputation platforms, widgets, settings, public collection, and unsubscribe remain separate slices.
