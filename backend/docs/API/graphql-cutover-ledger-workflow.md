...
The six subscription-billing application operations have completed direct consumer cutover to `BillingModule`. Status, plans, usage, checkout, portal, and trial acknowledgement are GraphQL-only; the exact-raw-body Stripe subscription webhook remains HTTP. Tenant locking, configured price and redirect allowlists, caller/provider idempotency, bounded provider behavior, and retirement evidence are frozen in the [Billing GraphQL cutover contract](contracts/billing-graphql-cutover.md).
...
All 26 email-template, SMS-template, administrator-email, contact-delivery, and Twilio-webhook operations are assigned. Seven email-template and eight SMS-template/message-info operations are implemented behind independent default-off read/mutation flags with PostgreSQL parity and frontend rollback coverage. Delivery and administrator operations still target GraphQL; Twilio callbacks remain signed HTTP with durable replay claims. Inbound tenancy comes from a globally unique organization-owned receiving number before the sender is matched inside that tenant. See [Messaging GraphQL cutover contract](contracts/messaging-graphql-cutover.md).
...
The five authenticated reputation-review CRUD operations are assigned to `ReputationReviewsModule`. Stable bounded reads, tenant-qualified joins and references, manual sentiment, row-locked response-state updates, CSRF, and rollback are frozen in the [Reputation reviews GraphQL cutover contract](contracts/reputation-reviews-graphql-cutover.md). The Reviews-page aggregate is independently assigned to `AnalyticsModule` with one tenant snapshot and a parameterized period in the [Reputation analytics GraphQL cutover contract](contracts/reputation-analytics-graphql-cutover.md). Review-request management and durable delivery are assigned to `ReputationRequestsModule` in the [Reputation request management GraphQL cutover contract](contracts/reputation-requests-graphql-cutover.md). Authenticated platforms, settings, widgets, and embed-code generation are implemented in `ReputationConfigurationModule` behind three independent frontend flags; public review and widget capabilities remain hardened HTTP. See the [Reputation configuration GraphQL cutover contract](contracts/reputation-configuration-graphql-cutover.md).

All 9 core dashboard and analytics operations plus the reputation aggregate are assigned to `AnalyticsModule`. Strict period/ID inputs, tenant and numeric normalization, bucket merging, selected-pipeline funnels, communication lifecycle counts, and the unresolved revenue/lifecycle/timezone/stage-history decisions are frozen in the [Analytics and dashboard GraphQL cutover contract](contracts/analytics-graphql-cutover.md). `dashboardAnalytics`, contact trends, deal performance, booking summary, communication stats, workflow performance, and reputation analytics are implemented behind seven independent default-off frontend flags. Conversion rates, revenue trends, and pipeline velocity remain blocked on explicit domain decisions rather than being mechanically copied from misleading legacy semantics.
