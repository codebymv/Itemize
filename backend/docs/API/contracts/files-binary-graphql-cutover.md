...
| --- | --- | --- |
| Signature source/template upload | Authenticated multipart HTTP plus organization membership and signature access | One `file`, 5 MiB maximum, full PDF parsing plus active-content/resource limits, optional pre-storage ClamAV inspection, forced `.pdf` key, tenant-owned draft/template checked before storage metadata is committed |
...
The NestJS signature receiver applies the same split to one PDF plus one resource-ID field. It caps the file at 5 MiB, bounds files/fields/parts and the legacy proxy body, verifies MIME, `%PDF-` bytes, full structural parsing, encryption policy, page count, page dimensions, object-graph complexity, stream expansion, embedded-image workload, and absence of active actions, scripts, forms, and attachments. The retained receiver enforces the same byte-level policy. Both receivers then cross an optional ClamAV `INSTREAM` boundary while the bytes are still isolated in bounded memory and before allocating a locator or writing a cleanup receipt. A detected file is rejected; a required but unavailable scanner returns a retryable `503`; and an unconfigured optional scanner reports `skipped`, never `clean`. The receiver generates an unguessable `.pdf` locator and fails closed in production without shared S3. Tenant ownership is checked before storage and again under a database row lock. Document uploads require `draft`, preserve any unrepresented active source, and append the new source at the next monotonic immutable version; a versioned source is never queued merely because it is superseded. Template replacement and template deletion enqueue their former locator. A lost row-lock race or database failure compensates by deleting the newly stored object. `SIGNATURE_FILE_UPLOADS_NESTJS_ENABLED` controls only the two uploads, while `SIGNATURE_FILE_READS_NESTJS_ENABLED` controls only the three authenticated private reads; both are default-off.
...
Authenticated and capability-authorized PDF reads now share one explicit conditional-delivery contract in NestJS and the retained rollback implementation. A request may select exactly one satisfiable `bytes=start-end`, `bytes=start-`, or `bytes=-suffix` range. Valid ranges return `206` with `Accept-Ranges: bytes`, an exact `Content-Range`, and the selected length; malformed, multiple, or unsatisfiable ranges return a non-enumerating `416` with `Content-Range: bytes */<total>`. The persisted SHA-256 becomes a strong ETag. Matching `If-None-Match` returns `304`, matching `If-Range` permits the partial response, and a stale `If-Range` deliberately returns the complete `200` representation. Local storage reads only the requested window; S3 performs `HeadObject` followed by a ranged `GetObject`.
...
Signature file lifecycle uses the same durable split. Migration `043_signature_file_deletion_jobs` records the tenant, optional document snapshot, and former locator in the metadata transaction. Migration 045 converts the tenant ID into an immutable snapshot rather than a cascading foreign key, allowing a cleanup receipt to outlive a draft-only organization deletion. Before an upload or generated completed artifact writes bytes, both implementations allocate an unguessable owned locator and commit a deletion receipt with a one-hour grace period. The successful authoritative metadata transaction removes that receipt; a crash, failed store, lost authorization race, or failed commit leaves cleanup authority behind. Retained multipart handling stays in bounded memory until registration, closing the former local-disk-before-receipt window. The worker preserves any locator still referenced by a document source, completed artifact, template, or immutable document version, accepts only a traversal-safe path below the private signature root or the exact configured S3 host and `signatures/` prefix, treats a missing local object as success, retries transient errors, and dead-letters unowned locators without egress. Draft PDF removal and whole-draft deletion enqueue every distinct current and historical source before clearing version rows; template replacement/deletion enqueue their source. Ordinary organization deletion refuses any non-draft signature evidence and snapshots all draft/template locators before cascading metadata. `VITE_SIGNATURE_FILE_MUTATIONS_GRAPHQL`, both retained-HTTP proxy flags, and worker scheduling remain default-off until production rehearsal.
...
- Private PDF delivery implements the same bounded single-range and ETag contract for local and S3 storage. Multiple/malformed/unsatisfiable ranges fail with `416`; stale `If-Range` falls back to a complete `200`, and matching `If-None-Match` returns `304`.
...
The PDF receiver now rejects malformed/encrypted input, active behavior and attachments, excessive object/stream/container graphs, unsupported non-image stream filters, extreme Flate expansion, oversized image workloads, more than 200 pages, and page dimensions above 14,400 points. A ClamAV-compatible pre-storage boundary is implemented in both runtimes, but production still has no scanner attached and therefore receives an explicit `skipped` verdict rather than a malware-clean claim. Before enabling the Nest upload route, deploy the scanner, set `SIGNATURE_MALWARE_SCAN_REQUIRED=true` on both rollback paths, and prove clean, infected, timeout, and unavailable canaries. Also complete the controlled private-S3 rehearsal and decide the time-based retention, export, and integrity-verification policy for non-draft evidence.
