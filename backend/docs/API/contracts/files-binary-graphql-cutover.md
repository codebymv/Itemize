...
Signature file lifecycle uses the same durable split. Migration `043_signature_file_deletion_jobs` records the tenant, optional document snapshot, and former locator in the metadata transaction. Before an upload or generated completed artifact writes bytes, both implementations allocate an unguessable owned locator and commit a deletion receipt with a one-hour grace period. The successful authoritative metadata transaction removes that receipt; a crash, failed store, lost authorization race, or failed commit leaves cleanup authority behind. Retained multipart handling stays in bounded memory until registration, closing the former local-disk-before-receipt window. The worker preserves any locator still referenced by a document, template, or immutable document version, accepts only a traversal-safe path below the private signature root or the exact configured S3 host and `signatures/` prefix, treats a missing local object as success, retries transient errors, and dead-letters unowned locators without egress. Draft PDF removal and whole-draft deletion enqueue every distinct active and historical source before clearing version rows; template replacement/deletion enqueue their source. `VITE_SIGNATURE_FILE_MUTATIONS_GRAPHQL`, both retained-HTTP proxy flags, and worker scheduling remain default-off until production rehearsal.
