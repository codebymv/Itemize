...
GraphQL owns file metadata, document lifecycle, logo deletion, and upload-intent orchestration. It does not carry multipart bodies, PDF streams, generated invoice PDFs, public logo bytes, or CSV transfers. Those protocols remain HTTP endpoints behind the same NestJS authentication, tenancy, capability, rate-limit, and observability services as GraphQL. `ContactTransfersModule` owns the two retained CSV routes, `InvoicesModule` owns retained invoice-PDF delivery, and `InvoiceLogoUploadsModule` owns the two retained invoice-logo uploads; the legacy origin proxies each boundary only behind its own default-off rollback flag.
