...
The generated consumer ledger and a source-scanning regression both prove that no shipped frontend called the aggregate REST route or currently calls the `contactProfile` GraphQL field. The unused Express aggregate and its stale child-query implementation are retired; `contactProfile` is the sole full-profile aggregate capability. The contact detail page intentionally composes its narrower contact, activity, and related-content fields in one `ContactDetailBootstrap` transport operation, so the full aggregate remains server-only until a UI actually renders its additional sections and is browser-tested.
