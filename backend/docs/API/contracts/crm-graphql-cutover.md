...
**Status:** Phase 1 contact CRUD, activities, bounded related content, aggregate contact profile, tag operations, pipeline-definition operations, canonical contact-email identity, canonical tag persistence, canonical pipeline-stage persistence, and the retained public-form boundary implemented; broader CRM characterization continues
...
Fresh PostgreSQL suites now cover contact CRUD/tenancy, profile authentication and forged-header denial, assignee denial, idempotent bulk tag changes, concurrent contact limits, canonical email migration repair and direct/API/GraphQL normalization, legal duplicate and email-less contacts, both CSV duplicate modes, deterministic public-form and booking resolution, deduplicated campaign preview/snapshots, canonical tag drift repair/projection/tenancy and case-insensitive races, pipeline/deal CRUD and lifecycle, cross-tenant deal references, invalid stages, stage-in-use protection, default concurrency, form CRUD/fields/duplication/submissions, same-email public submission concurrency, and concurrent form limits. Public-form cases additionally prove global identity and ambiguous-slug denial, typed normalization and invalid/unknown/oversized rejection without writes, safe redirects, tenant/object database constraints, durable trigger and notification fan-out, worker delivery with the stable idempotency key, and replay-safe historical repair.

The NestJS `ContactsModule` now implements `contacts`, `contact`, `createContact`, `updateContact`, `deleteContact`, `bulkUpdateContacts`, `bulkDeleteContacts`, `contactActivities`, `addContactActivity`, `contactContent`, and `contactProfile`. `TagsModule` implements all five tag operations, and `PipelinesModule` implements all five pipeline-definition operations. Fresh-PostgreSQL tests prove dual REST/GraphQL contact parity plus canonical tag counts and suggestions, projection-preserving rename/delete, duplicate/color/CSRF/tenant denial, deterministic pipeline aggregates and nested board reads, stage normalization/reordering, explicit description clearing, concurrent defaults, in-use stage rejection, and protected deletion. Contact mutation cases additionally prove normalized creation, serialized plan enforcement, assignee membership, omitted-versus-null updates, exact deletion, bounded bulk changes, activity reads/writes, related content, aggregate profile composition, and transactionally coupled workflow/activity effects.

The shared frontend contact API has separate strict opt-ins for reads, single-row mutations, bulk mutations, activity timeline reads/writes, and related-content reads, preserves the existing consumer shape, fetches and forwards CSRF for GraphQL writes, surfaces GraphQL error messages in contact modals, and defaults every path to REST. The pipeline board adds independent `VITE_PIPELINE_READS_GRAPHQL` and `VITE_PIPELINE_MUTATIONS_GRAPHQL` switches; its adapter preserves the current snake-case pipeline/deal shape and obtains CSRF for definition writes. There is no standalone tag API consumer to switch. Related content fails closed instead of silently truncating when any collection exceeds the 100-row GraphQL bound. All 101 frontend cases pass, including pipeline mapping/rollback defaults and the retained public-form condition and redirect behavior. The contact staging rehearsals passed GraphQL-to-REST rollback without data repair. The five contact and two pipeline flags remain disabled in deployed builds.
...
1. remaining CSV boundaries and profile failure-injection/consumer behavior have complete tests;
2. tag/pipeline staging operation and board rollback journeys, plus form GraphQL/browser journeys, pass semantic-parity gates; profile browser coverage becomes required if a consumer is introduced.
