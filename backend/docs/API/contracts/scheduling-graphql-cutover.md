...
The current database guarantees only `(organization_id, slug)` uniqueness while public routes query slug alone. Public page, slot, and create lookups now fail closed unless exactly one active calendar matches, so ambiguity cannot select the wrong tenant. A globally unique slug or organization-qualified public URL remains the stable-identity cutover requirement.
...
1. public calendar identity is globally stable and unambiguous, and cancellation tokens are hashed, expiring, and redacted;
2. the remaining calendar-delete null/reference semantics are frozen; authenticated booking create/reschedule/cancellation references and lifecycle transitions, booking read projections, calendar definition/availability/override semantics, and the shared slot policy are now covered;
3. provider tokens are encrypted with rotation and concurrent refresh protection;
4. sync becomes durable idempotent work, populates normalized external busy intervals, and makes `pull`/`both` behavior match advertised settings;
5. worker-consumer retry/idempotency is frozen for the now-versioned create/reschedule/cancellation events;
6. create/reschedule plus the remaining GraphQL operations, retained HTTP protocols, and critical calendar/public-booking/provider journeys pass semantic parity and rollback tests.
