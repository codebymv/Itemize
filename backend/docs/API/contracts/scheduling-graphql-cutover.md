# Scheduling GraphQL cutover contract

**Status:** Authenticated calendar definition/availability, booking read/write, and calendar connection/sync-management consumers permanently cut over to GraphQL with their 18 Express routes retired; the shared authoritative slot policy, public booking identity/capabilities, retained OAuth/provider HTTP protocols, and durable bidirectional calendar synchronization remain implementation/integration-proven

**Evidence date:** 2026-07-28

## Decision

Authenticated calendar, booking, connection, and durable sync-management operations move to GraphQL. Anonymous booking-page/slot/create/cancel routes remain rate-limited HTTP protocols. Google OAuth initiation/callback and live provider-calendar discovery remain HTTP provider protocols during this checkpoint; they perform browser redirects or live provider I/O rather than tenant-database mutations.

The authoritative assignments for all 25 scheduling operations are in `graphql-operation-overrides.json`.

## Ownership

| Domain | NestJS owner | Target operations |
| --- | --- | --- |
| Calendars and availability | `CalendarsModule` | `calendars`, `calendar`, `createCalendar`, `updateCalendar`, `deleteCalendar`, `replaceCalendarAvailability`, `upsertCalendarDateOverride`, `deleteCalendarDateOverride` |
| Authenticated bookings | `BookingsModule` | `bookings`, `booking`, `createBooking`, `cancelBooking`, `rescheduleBooking` |
| Anonymous booking protocol | `PublicBookingsModule` | retained HTTP `getPublicBookingPage`, `getPublicBookingSlots`, `createPublicBooking`, `cancelPublicBooking` |
| Connection and sync management | `CalendarIntegrationsModule` | `calendarConnections`, `disconnectCalendar`, `updateCalendarConnection`, `requestCalendarSync`, `calendarSyncStatus` |
| Provider HTTP protocols | `CalendarOAuthModule` | retained HTTP `beginGoogleCalendarConnection`, `googleCalendarCallback`, `providerCalendars` |

## Authentication and tenancy

Every GraphQL operation uses verified user and organization context. Calendar, booking, contact, assignee, connection, and sync-event projections must belong to that context; an outside-organization ID returns `NOT_FOUND`.

- Anonymous routes derive the organization from a globally unambiguous public calendar identity, never from a request header.
- A calendar assignee must be a current member of its organization.
- A booking's calendar, contact, and assignee form one organization-consistent tuple.
- Connections are user-owned inside an organization. An organization peer cannot read or operate another user's provider tokens.
- The OAuth callback accepts identity and tenant claims only from signed, short-lived state and rechecks current organization membership before exchanging the code.

Legacy OAuth state formerly contained unsigned JSON with trusted `userId`, `organizationId`, and `returnUrl`, allowing forged connection ownership and an open redirect. State is now HMAC-signed, expires after ten minutes, contains a nonce, and restricts redirects to local frontend paths. The target should use a dedicated rotated secret and may store/claim the nonce when a provider does not guarantee one-time authorization codes.

## Calendar definition and availability

Calendar timezone is a valid IANA zone. Durations, buffers, notice, future horizon, and reminders are bounded non-negative integers; duration is greater than zero. Update inputs distinguish omitted fields from explicit null.

Recurring windows use calendar-local weekday/time values. Each window has `start < end`, windows do not overlap on the same weekday, and overnight availability requires an explicit product rule rather than accidental SQL behavior. An available date override requires a valid start/end window; an unavailable override has no window. Replacement commits atomically.

Delete locks the organization-owned calendar and the same booking namespace used by booking creation. Pending or confirmed future bookings block deletion. The legacy route now has this transaction boundary.

## Booking identity and lifecycle

Booking timestamps are absolute instants plus a valid display timezone. `endTime` must be later than `startTime`. The target lifecycle is:

```text
pending -> confirmed -> completed
pending|confirmed -> cancelled
pending|confirmed -> no_show
```

Rescheduling applies only to active bookings. Cancellation and rescheduling are explicit mutations with conflict behavior for invalid states. Repeated cancellation must not emit duplicate domain events.

For every create or reschedule, the service takes a transaction-scoped lock for the calendar, evaluates collision and availability policy, writes the booking, and writes a durable domain event in the same transaction. Legacy manual and public creation plus rescheduling now share the collision lock. A real-PostgreSQL race proves two simultaneous reservations for the same calendar interval commit exactly one row.

Collision uses half-open intervals: `[start, end)`. Adjacent bookings are permitted only when configured buffers leave them non-conflicting. One PostgreSQL policy now applies buffers, recurring windows, date overrides, minimum notice, maximum future horizon, calendar activity, timezone/DST rules, and normalized external busy intervals to retained manual/public writes and authenticated GraphQL create/reschedule. Slot discovery calls that same policy.

## Public booking HTTP contract

The public routes remain HTTP because they are public resource/capability protocols. They require abuse limits, request/body bounds, non-enumerating errors where appropriate, and operational correlation without logging attendee PII or cancellation tokens.

NestJS `PublicBookingsModule` owns these routes directly. The retired Express proxy and `PUBLIC_BOOKINGS_NESTJS_ENABLED` flag are not current rollback controls. The fresh PostgreSQL suite is `backend/test/integration/public-bookings.integration-spec.ts`.

Public and organizer booking create/reschedule use the shared `bookingTimezone` validator: supplied values must be strings, trimmed, nonempty, at most 100 characters, and accepted by Intl's timezone validator. An omitted/null public timezone continues to use the calendar default. Public failures retain the HTTP error envelope with `INVALID_TIMEZONE`; GraphQL retains `BAD_USER_INPUT` and field metadata.

`POST /api/bookings/public/book/:identifier/status` takes `{ token }` in the body, keeping the private capability out of URLs. It returns only start/end instants, timezone, and status, with `Cache-Control: no-store`. The token hash must match the resolved active calendar and tenant and remain unexpired. Malformed, wrong-calendar, expired, and revoked tokens all return a non-enumerating 404. No attendee, contact, or internal booking identifiers are returned. Cancellation still revokes the token; a revoked token does not reveal whether cancellation caused the miss.

The already-open confirmation page refreshes immediately, every 30 seconds while visible, and on focus/visibility return. Requests do not overlap, and unmount/cancellation stops refreshes. It displays the returned booking timezone, including reschedules that change timezone. A failed lookup labels the details as last-known and hides cancellation until a successful refresh. Legacy invalid stored display zones fall back explicitly to UTC. Tokens remain in page memory; this change does not add persistent booking links or reload recovery.

Migration `031_booking_public_capabilities` gives every calendar an immutable-format, globally unique `cal_` public ID. Public page, slot, create, and cancellation lookups resolve that ID first. Existing slug URLs remain a compatibility fallback only when exactly one active calendar matches, so cross-organization ambiguity fails closed. Authenticated REST and GraphQL calendar projections include the public ID, and the shipped calendar link controls now generate public-ID URLs.

Slot calculation is now server-authoritative and timezone-aware. The retained endpoint accepts at most 31 inclusive calendar days and returns only concrete UTC start/end instants, never recurring windows, overrides, or booked intervals. Public creation transactionally rechecks the same PostgreSQL rules after acquiring the calendar booking lock, so a displayed slot is never trusted as current. A short-lived availability version/claim may still be added to improve stale-slot diagnostics, but it is not an authorization boundary.

Public booking contact reuse is case-insensitive and serialized by organization/email. Email is not declared a universal CRM identity; this reuse policy is specific to public booking. Contact creation, booking creation, and the durable booking event share a deliberate transaction/error policy.

Public creation returns a 256-bit cancellation capability exactly once and stores only its SHA-256 hash with an expiry of booking end plus one day. Raw tokens are forbidden by a PostgreSQL constraint and excluded from authenticated REST and GraphQL projections. Cancellation binds the hash to the resolved calendar and confirmed state, rejects malformed, expired, wrong-calendar, and replay attempts with the same non-enumerating miss, and clears the hash/expiry on success. Authenticated cancellation also revokes the capability, while authenticated rescheduling moves its expiry with the booking. Migration repair hashes still-useful legacy raw tokens, clears every raw value, drops the raw-token index, deduplicates conflicting historical capabilities, and is idempotent.

## Provider connections and sync

Migration `032_calendar_token_encryption` converts every stored provider credential into a versioned AES-256-GCM envelope, records a monotonic token generation, rejects future plaintext access/refresh tokens in PostgreSQL, and scopes provider-account uniqueness by organization as well as user. `CALENDAR_TOKEN_ENCRYPTION_KEYS` retains readable key versions while `CALENDAR_TOKEN_ACTIVE_KEY_ID` selects the write key; reads lazily rewrap old envelopes under the active key. Production refuses Google OAuth configuration without a valid keyring, while disposable development/test environments derive an isolated fallback from their non-production JWT secret.

OAuth callback storage encrypts before persistence and tokens remain excluded from DTOs/logs/errors. Provider calendar reads and the calendar-sync worker load credentials through one row-locked transaction. The first expired-token caller refreshes and commits encrypted access/refresh envelopes; concurrent followers re-read the committed generation and reuse it instead of issuing another provider refresh.

`GOOGLE_CALENDAR_REDIRECT_URI` selects the calendar OAuth callback independently from Google sign-in. Staging must set it explicitly to the staging backend callback registered on its OAuth client; production mode rejects insecure configured callbacks. The production and local callback defaults remain compatibility fallbacks, but a provider rehearsal must never rely on them.

Rotation is additive: generate a new 32-byte key, add it to `CALENDAR_TOKEN_ENCRYPTION_KEYS`, switch `CALENDAR_TOKEN_ACTIVE_KEY_ID`, deploy, and retain prior keys while reads lazily rewrap existing envelopes. Removing an old key before every envelope using it has rotated makes those credentials intentionally unreadable and requires provider reconnection.

Connection settings validate provider, sync direction, and selected calendar identifiers. Disconnect defines whether remote events are retained or removed. Provider calls do not run inside a GraphQL resolver transaction.

`CalendarIntegrationsModule` exposes only safe connection and job projections. Queries and mutations scope every row to both the verified organization and current user; organization peers cannot enumerate or operate one another's provider connections. Access/refresh tokens, provider account IDs, sync cursors, job idempotency keys, lease ownership, and claim metadata are absent from the GraphQL schema. Settings, disconnect, and sync requests require CSRF. The five database-backed management operations are unconditional GraphQL consumers, while OAuth initiation/callback and live provider-calendar listing continue through HTTP.

Migration `033_calendar_sync_jobs` adds one active durable job per connection, durable request-key aliases, direction/calendar snapshots, leased `SKIP LOCKED` claims, bounded exponential retry, redacted errors, fenced completion, and visible retry/dead-letter state. The retained request endpoint now returns `202` after enqueueing and never calls Google inline; sync status includes the ten newest safe job projections. The worker remains explicitly default-off behind `CALENDAR_SYNC_JOBS_ENABLED`.
NestJS now carries a faithful port of this worker (`CalendarSyncJobsModule`:
claim/lease/fence SQL, push with deterministic remote identity and the
409/404 provider fallbacks, pull busy-window replacement with the
stale-claim fence, backoff, redaction, and connection error bookkeeping),
default-off behind `CALENDAR_SYNC_NEST_JOBS_ENABLED`; the runtime
validator rejects enabling both flags in one runtime. A dual-runtime
fresh-PostgreSQL parity suite drives both workers over identical seeded
jobs with injected provider boundaries, and cross-runtime unit parity
pins the normalization, backoff, and redaction primitives.

Push uses deterministic provider event IDs, repairs an ambiguous create replay through update, upserts one connection/booking sync identity, and treats a remote `404` during cancellation as a completed delete. Pull reads up to ten 250-event pages per selected provider calendar over the bounded active-calendar horizon, resolves all-day dates in the provider timezone, excludes Itemize-owned, cancelled, and invalid events, and atomically replaces normalized external busy intervals for active internal calendars assigned to the connection owner. `push`, `pull`, and `both` therefore match the stored direction. Provider failures retry the durable job and never make a partially failed push appear successful.

## Workflow and event boundary

Booking mutations must write versioned `booking.created`, `booking.cancelled`, and `booking.rescheduled` events to the durable outbox described by the workflow execution contract. Resolvers do not invoke the automation engine or email/calendar providers.

The durable workflow queue accepts the canonical `booking_created`, `booking_cancelled`, and `booking_rescheduled` trigger vocabulary. GraphQL create, cancel, and reschedule payloads are frozen at version `1`. Create and cancel use deterministic per-booking domain keys; every successful reschedule has a unique key because each transition is independently consumable. The long-term worker-consumer retry and idempotency contract remains a release gate.

## Required parity scenarios

| Area | Required scenarios |
| --- | --- |
| Calendars | CRUD, assignment denial, timezone/numeric validation, omitted/null inputs, delete/create race, active-booking protection, tenant denial |
| Availability | window validation/overlap, atomic replacement rollback, override rules, DST gaps/folds, tenant denial |
| Booking lists | filters, strict timestamp/page inputs, deterministic ordering, bounded relation projections, tenant denial |
| Booking writes | invalid interval, adjacent/overlap/buffer cases, simultaneous create, create-versus-reschedule, lifecycle conflicts, reference denial, transaction rollback |
| Public booking | global identity, rate/body limits, authoritative slots, min notice/horizon, DST, same-email race, collision race, token wrong-calendar/expiry/replay/redaction |
| OAuth | signed/expired/tampered state, membership removed during flow, safe return path, provider denial/error, code replay, token encryption/redaction |
| Sync | connection ownership, concurrent refresh, job idempotency, push/update/delete, selected calendars, partial retry, provider rate limit, pull/both policy |
| Events | outbox atomicity, payload version, one event per transition, worker retry/idempotency, no provider calls from resolver |

## Current evidence and exit gate

Fresh PostgreSQL suites cover calendar CRUD/tenancy, availability replacement, overrides, booking CRUD/tenancy, globally unique public calendar IDs with ambiguous-slug denial, one-time public capability issuance, hash-only persistence, expiry, wrong-calendar/replay denial, authenticated reschedule/cancel alignment, legacy-token migration repair, provider-token encryption repair, plaintext rejection, single-winner concurrent refresh, sync-request coalescing, competing worker claims, direction dispatch, retry/redacted dead-lettering, and normalized external-busy replacement. Unit tests cover signed OAuth state, expiry, tampering, redirect normalization, encryption authentication, keyring validation, key rotation, deterministic provider replay repair, and idempotent remote deletion.

The first bounded NestJS checkpoint implements authenticated `calendars` and `calendar` reads. Both require verified organization context, qualify every calendar lookup by organization, expose an assignee name only through a current membership, return deterministic list ordering and confirmed-booking aggregates, and project ordered recurring availability plus current/future date overrides on detail.

The second checkpoint adds only `createCalendar` and `updateCalendar`. Both are CSRF-protected and organization-scoped. Create applies normalized legacy-compatible defaults, validates IANA timezone and bounded settings, validates and deterministically orders optional recurring windows, creates Mon-Fri defaults when windows are omitted, verifies assignee membership, and enforces the organization calendar limit under an advisory transaction lock. Update distinguishes omission from explicit nullable clearing, locks the organization-owned row, validates the final assignment mode/assignee tuple, preserves omitted fields, and reloads the complete retained projection.

The third checkpoint adds `replaceCalendarAvailability`, `upsertCalendarDateOverride`, and `deleteCalendarDateOverride`. Replacement validates and deterministically orders at most 100 local weekday windows before entering a transaction, rejects overlaps and non-increasing intervals without deleting existing rows, locks the organization-owned calendar, and replaces its schedule atomically. Override upsert validates real ISO calendar dates and requires exactly one valid local-time window for available dates and no window for unavailable dates. Override deletion scopes both the child ID and its parent calendar through the verified organization, closing the legacy child-ID-only tenant boundary. All three mutations require CSRF.

The fourth checkpoint added `deleteCalendar`. The resolver requires CSRF and verified organization context. Delete and retained booking writes now acquire the transaction-scoped calendar-booking advisory lock before any calendar row lock or booking insert, and booking writers recheck the calendar after acquiring that namespace lock. The repository then locks the organization-owned calendar, rejects future `pending` or `confirmed` bookings, and deletes the calendar plus cascade-owned schedule/history only when that protected namespace is clear. Foreign IDs remain concealed as `NOT_FOUND`, while the active-booking conflict has stable reason `UPCOMING_BOOKINGS`.

The calendar-definition cutover checkpoint removes all three calendar rollout flags and makes these eight operations unconditional GraphQL consumers. Their Express router and handlers are deleted, retirement canaries require `404` for every former method/path, and booking fixtures no longer create calendars through the retired transport. Authenticated booking routes, anonymous booking protocols, OAuth/provider protocols, connection/sync management, and their independent rollout boundaries remain unchanged.

The fifth checkpoint adds authenticated `bookings` and `booking` queries. Both require verified organization context, qualify booking and calendar rows by organization, expose contacts and assignees only through organization-consistent joins, validate status, calendar, contact, assignee, DateTime range, and bounded page inputs, and order lists by `start_time DESC, id DESC`. Foreign IDs remain concealed as `NOT_FOUND`, and raw cancellation capabilities remain excluded from GraphQL projections.

The sixth checkpoint adds CSRF-protected, organization-scoped `cancelBooking`. It locks the tenant-owned booking, permits only `pending` or `confirmed`, commits status, timestamp, normalized optional reason, and exactly one durable `booking_cancelled` trigger in one transaction, and rejects replay without another mutation or event. Missing and foreign IDs remain concealed.

The seventh checkpoint adds CSRF-protected, organization-scoped `createBooking` and `rescheduleBooking`. Both acquire the canonical calendar-booking advisory lock before calendar or booking row locks, validate the organization-owned calendar and every contact/assignee reference, apply the shared authoritative availability policy, and write the booking transition plus one version-1 durable event in the same transaction. Authenticated create does not issue an anonymous cancellation capability. Reschedule applies only to `pending` or `confirmed`, preserves timezone when omitted, and moves any existing public capability expiry to the new end plus one day. Two simultaneous creates for one interval commit exactly one booking.

The authenticated-booking cutover checkpoint removes all three booking rollout flags and makes list, detail, create, cancel, and reschedule unconditional GraphQL consumers. The mixed Express router now owns only public booking discovery, slot calculation, creation, and capability cancellation; the five authenticated handlers are deleted and explicit retirement canaries require `404`. The authoritative Nest tests retain tenancy, validation, collision, event, capability-expiry/revocation, and calendar-delete race coverage without depending on the removed transport.

The calendar-integration management cutover checkpoint removes the final integration rollout flag and makes connection list/update/disconnect plus sync enqueue/status unconditional GraphQL consumers. Their five Express handlers are deleted and explicit retirement canaries require `404`. The mixed router continues to own Google OAuth initiation, the provider callback, and live provider-calendar discovery because those are browser/provider HTTP protocols. Authoritative Nest PostgreSQL coverage retains current-user and organization scoping, CSRF, validation, idempotent durable enqueue, safe bounded status, and non-enumerating denial for peer-user and foreign-organization connection IDs.

The eighth checkpoint installs migration `030_booking_availability_policy`. `booking_slot_policy_reason` is the single write-time authority for retained manual/public create, retained reschedule, and GraphQL create/reschedule. `booking_available_slots` generates the public duration grid from effective local windows, lets date overrides replace recurring windows, omits DST gaps, deterministically emits one instant for repeated wall times, and filters through the same policy. The policy returns stable reasons for invalid ranges/duration, inactive calendars, windows/overrides, DST transitions, notice, horizon, buffered booking collisions, and external busy intervals. Normalized provider-busy rows enforce calendar/connection/organization consistency in PostgreSQL. Fresh PostgreSQL proves bounded non-leaking slot output, selected-slot revalidation, buffers, notice, horizon, overrides, DST gaps/folds, external busy suppression and tenant binding, and fail-closed ambiguous public slugs.

The ninth checkpoint installs migration `031_booking_public_capabilities`. It backfills constrained globally unique `cal_` identifiers, exposes them across retained and GraphQL calendar projections, and preserves only an exactly-one-match slug fallback. It migrates still-live legacy cancellation tokens to SHA-256 hashes, sets end-plus-one-day expiry, clears raw values, forbids future raw storage, and enforces paired hash/expiry state. Public creation issues the raw capability once; public and authenticated cancellation revoke it; rescheduling moves its expiry. Fresh PostgreSQL proves public-ID routing, ambiguous fallback denial, hash-only persistence, expiry, wrong-calendar/replay rejection, lifecycle revocation, and idempotent historical repair.

The tenth checkpoint installs migration `032_calendar_token_encryption`. It migrates plaintext Google credentials into authenticated versioned envelopes, enforces encrypted storage, supports active-key rotation with retained read keys, and adds a token generation. OAuth callback writes encrypt before persistence. Provider calendar reads and sync serialize credential refresh with a PostgreSQL row lock, preserve rotated refresh tokens, and persist only the winning encrypted generation. Fresh PostgreSQL proves concurrent callers make one provider refresh, followers reuse the result, migration reruns preserve active envelopes, and plaintext writes fail closed.

The eleventh checkpoint installs migration `033_calendar_sync_jobs`. Manual sync is now an idempotent enqueue-only boundary. Leased workers recover expired claims, fence stale completions, retry with redacted diagnostics, dead-letter at a bounded attempt count, and implement snapshotted `push`, `pull`, or `both` behavior. Push delivery has deterministic remote identity and cancellation cleanup; pull populates the external-busy table already enforced by authoritative booking availability. Fresh PostgreSQL proves request replay/coalescing (including replay after completion), one winner across competing workers, both-direction dispatch, retry/dead-letter state, and stale busy-row removal.

The twelfth checkpoint adds `calendarConnections`, `updateCalendarConnection`, `disconnectCalendar`, `requestCalendarSync`, and `calendarSyncStatus`. Reads expose only current-user rows in the verified organization. Settings reject nulls, unsupported directions, duplicates, blank/oversized IDs, and lists over 100 before a user-and-organization-qualified update. Sync enqueue preserves durable idempotency/coalescing and returns immediately without provider I/O. Status returns bounded aggregates plus the ten newest safe jobs. All mutations require CSRF; foreign organization and peer-user IDs share `NOT_FOUND`. The frontend adapter preserves the existing response casing and keeps OAuth/provider discovery on HTTP.

The frontend `/calendars/:id` consumer routes definition and availability writes independently, preserving existing response shapes and REST defaults. Twelve focused calendar service cases, fourteen adapter/config cases, and eleven fresh-PostgreSQL calendar cases prove normalization, null/omission behavior, custom availability, atomic overlap rejection, available/unavailable override rules, plan limits, canonical booking/delete lock ordering, the retained-create-versus-GraphQL-delete race, delete protection/cascade behavior, CSRF, assignee and tenant denial, independent flag selection, REST interoperability, and retained-shape mapping. Eighteen focused booking service cases, twelve fresh-PostgreSQL booking cases, and twelve booking adapter/selection cases now also prove create/reschedule/cancellation normalization, tenant-reference and CSRF denial, simultaneous slot exclusion, lifecycle conflicts, atomic versioned event persistence, retained-shape mutation mapping, and three independent REST-default routing boundaries. Five focused integration-service cases, four fresh-PostgreSQL GraphQL cases, and seven frontend adapter/selection cases cover the new connection slice. The complete gates pass at 159/159 NestJS focused cases, 98/98 NestJS fresh-PostgreSQL cases, 195/195 frontend cases, and 489/489 legacy fresh-PostgreSQL cases. The active calendar and booking read consumers, booking cancellation action, weekly-availability writer, and calendar deletion action have passed their staging browser and independent flag-off rollback rehearsals. Booking create/reschedule, date-override, and calendar-integration adapters remain implementation/integration-proven because the shipped interfaces have not completed their corresponding provider-backed browser gates.

The calendar-integration staging preflight deployed backend `197d9d80-6ed9-4772-8ba0-328b1586e79e` and GraphQL `fbbe0d43-cd97-4ba0-aad1-71777ab90cd5`. Numbered migrations 030 through 033 applied successfully; the restarted backend registered all API routes, direct GraphQL returned `readiness: ready`, and direct plus legacy-proxy calendar-operation probes reached `UNAUTHENTICATED` without schema-validation errors. The durable queue reported zero active and zero dead-letter jobs. Google client credentials, the explicit staging redirect, the calendar keyring, and the worker flag remain absent, so provider I/O is disabled. The provider-backed portion now follows the staging-only calendar sync sandbox runbook and remains blocked only on a restricted Google OAuth client plus interactive sandbox consent.

Committed cancellation deployment `04a5cc20-a0cd-4570-a597-ec6d10eef149` passed its authenticated real-browser and independent REST rollback gate on 2026-07-19. With booking reads and cancellation enabled only in a local harness, booking `5` cancelled through successful proxy/NestJS `CancelBooking` request `0a089762-ce6d-4f1c-bcdc-77513a4238d1`; the visible row changed to `cancelled`, and PostgreSQL held the normalized reason plus exactly one version-1 event. Disabling only `VITE_BOOKING_MUTATIONS_GRAPHQL` preserved the same session and cancelled booking `6` through retained `PATCH /api/bookings/6/cancel` (`200`, request `79f95b11-cc24-4864-8cdc-fa3e3a70f9a6`) with one retained event and no later cancellation mutation. A variable-only Railway rebuild selected stale Git source and temporarily returned proxy `404`; explicitly restoring current local source recovered the gate, so staging variable changes must be followed by an explicit current-source deployment until Railway's Git source is advanced. Cleanup verified zero fixture rows, removed localhost CORS, stopped the harness, and left healthy clean backend deployment `3e439dd9-8e64-4cf8-96d8-712c2682fc12`. All booking flags remain default-off in deployed builds and production was untouched.

The completed browser gate ran on 2026-07-18 against GraphQL deployment `9705daa8-115f-4a44-acff-87919b3ee38a` through backend gate deployment `b7e0d641-4c1e-41f4-be13-f19c87ef84f5`. A disposable verified account first rendered the empty list, then created a distinctive calendar through retained REST. A real navigation reload rendered that persisted row through nonempty `CalendarReads`, and the Settings action opened `/calendars/3` through `CalendarRead`, including five recurring weekday windows. Proxy and NestJS events paired by request ID with HTTP `200` and zero errors, including list request `4811017d-e568-47e7-904f-20926f14eb41` and detail request `62871253-558f-4684-a4c7-988860483aa4`.

The detail consumer then changed the calendar name through `PUT /api/calendars/3` and Monday availability through `PUT /api/calendars/3/availability`. The browser gate exposed that native time edits were not reaching React state under real Chromium when wired only through the prior change-event path; the controls now use the native input event, and a fresh detail reload retained the new `10:00` start. Restarting only the frontend with `VITE_CALENDAR_READS_GRAPHQL=false` retained the same session and rendered the persisted name through `GET /api/calendars` and the name plus updated availability through `GET /api/calendars/3`, all without data repair. Re-enabling the flag rendered that same final state through paired zero-error `CalendarReads` request `689d320c-6ed0-4502-bf2b-6431b7ffda60` and `CalendarRead` request `a370e96a-d4c5-4762-a9cb-884ffa347235`. Cleanup deleted one disposable organization and user, stopped the local harness, removed temporary localhost CORS, and restored clean backend deployment `92940159-98ff-41a8-a2c9-306109530890`; `EXTRA_CORS_ORIGINS` is absent, the calendar flag remains default-off, and production was untouched.

The definition-mutation browser gate then passed against GraphQL deployment `4b6c8105-7364-40a8-98bd-98f0b251d490` through backend gate deployment `aa8a4f61-a8d4-4874-b6cb-d38dadf69370`. A disposable real-browser account created and updated one calendar through GraphQL while reads remained independently enabled, then disabled only `VITE_CALENDAR_MUTATIONS_GRAPHQL` and updated that same row through retained REST without data repair. Cleanup removed all fixture rows, temporary local access was removed, and clean backend deployment `c8f9ee32-a5aa-4094-acb6-a1913fb31663` restored committed source. Both prior calendar flags remain default-off and production was untouched.

The weekly-availability mutation gate passed against GraphQL deployment `2f0078e1-bccc-421b-acb0-f27055dce474` through backend gate deployment `dac4d988-7261-4501-9dd9-117724c4fdd7`. A disposable account registered and signed in through the real credential forms, created calendar `5` through `CreateCalendar`, and saved distinctive Monday hours of `10:15`–`15:45` through `ReplaceCalendarAvailability`. The UI reported success, a full reload retained the values through `CalendarRead`, and the proxy recorded zero-error mutation request `448a3cbd-c6cb-477e-a929-644757bf98d1` as HTTP `200`. Restarting only the frontend with `VITE_CALENDAR_AVAILABILITY_MUTATIONS_GRAPHQL=false` kept calendar reads and definition mutations on GraphQL; the same settings control saved `11:30`–`14:00` through `PUT /api/calendars/5/availability` (`200`, request `90acc609-6875-4cd2-a8c8-d58bc0872d09`), and another GraphQL-backed reload retained the REST-written state without repair or a later GraphQL availability mutation. Transactional cleanup returned zero disposable users, organizations, memberships, or calendars. The harness stopped, temporary localhost CORS was removed, and clean backend deployment `a7addfc5-3a75-40b8-b434-848959ed01fb` restored committed source; direct and proxied GraphQL probes both returned `Query`, all three calendar flags remain default-off, and production was untouched.

The calendar-deletion gate passed against GraphQL deployment `c6ef5954-5971-4eca-8bdb-c4858f0e8393` through backend gate deployment `e7e3143f-a7d5-44b6-82dc-77ae77652660`. A disposable account signed in through the real credential form and created three calendars through GraphQL. Deleting calendar `6` removed the row with a success notification and zero-error `DeleteCalendar` request `e44afb26-6c7f-49c6-809e-db9df8e2a8d1`. Calendar `7` had a future confirmed booking; `DeleteCalendar` request `c6ae0343-d442-4e60-b112-cc04a993e732` returned `BAD_USER_INPUT` with stable service reason `UPCOMING_BOOKINGS`, and the row plus its `1 upcoming` indicator remained. The current page collapses that domain message to `Failed to delete calendar`; exposing the actionable server copy is a non-blocking UI follow-up. Restarting only the frontend with `VITE_CALENDAR_MUTATIONS_GRAPHQL=false` then deleted calendar `8` through `DELETE /api/calendars/8` (`200`, request `W1_sDqneQMWYN0PpnpoFkQ`) without data repair. Transactional cleanup verified zero disposable users, organizations, memberships, calendars, or bookings. The harness stopped, temporary localhost CORS was removed, and clean backend deployment `8b238070-1085-4977-8b09-12761f8c3e4d` restored committed source; direct and proxied GraphQL probes both returned `Query`, all calendar flags remain default-off, and production was untouched.

The authenticated booking-read gate passed against committed GraphQL deployment `c50d8e48-93b7-4a23-ab82-f1386af29274` through backend gate deployment `8dc50b3a-37e4-4633-8520-a2e9cfd21d06`. A disposable verified account with one calendar and two bookings signed in through the real credential form. The `/bookings` page rendered both retained-shape rows through zero-error `BookingReads` request `c60069bf-d703-4350-bb44-43ca5fd8896a`; selecting Confirmed rendered only the confirmed row through request `02aa97e6-597b-4906-9a9f-b4f4ed4385b2`, and selecting Pending rendered only the pending row through request `3af754ce-a9e5-48ba-aad8-aa18c37bcf82`. Because the shipped app has no booking-detail route, authenticated staging request `c8c131fb-22b9-49ce-90db-46c6f82c4159` directly compared `BookingRead` with retained `GET /api/bookings/3` and matched identity, title, status, calendar, contact, assignee, and custom-field projections. Restarting only the frontend with `VITE_BOOKING_READS_GRAPHQL=false` kept the same session and rendered both rows through retained `GET /api/bookings` (`200`, Railway request `oQofq4qAR0e5AeD6WUN5dQ`) without data repair. Transactional cleanup verified zero disposable users, organizations, memberships, calendars, contacts, or bookings. The harness and browser tab stopped, localhost CORS was removed, and clean backend deployment `7d4dce53-f735-4151-b153-42bf5764a590` restored committed source; both health routes returned `200`, direct and proxied booking probes reached `UNAUTHENTICATED` rather than schema validation, `VITE_BOOKING_READS_GRAPHQL` remains default-off, and production was untouched.

The scheduling slice is not ready for traffic until:

1. the remaining calendar-delete null/reference semantics are frozen; authenticated booking create/reschedule/cancellation references and lifecycle transitions, booking read projections, calendar definition/availability/override semantics, the shared slot policy, and public capability contract are now covered;
2. worker-consumer retry/idempotency is frozen for the now-versioned create/reschedule/cancellation events;
3. create/reschedule plus the remaining retained HTTP protocols and critical calendar/public-booking/provider journeys pass semantic parity and rollback tests;
4. a sandbox Google push/pull/rate-limit/revocation rehearsal passes before the default-off worker is enabled.


## Paid public intake (2026-09-11)

Public form/calendar discovery and new intake require the shared paid-entitlement
predicate: a paid plan with active status or a non-expired trial. Intake locks the
organization for the transaction so subscription changes cannot race admission.
Ineligible resources return 404 without deleting or changing their publication state.
Existing booking capability status/cancellation remains available while the token
is valid. Queued workflow notifications retain the existing paid-access claim rule.


## Shared contact quota (2026-09-11)

Contact creation uses the shared organization transaction lock and stored quota.
Public forms/bookings preserve intake with no new contact when full, while reusing
existing email-matched contacts. Explicit chat conversion returns PLAN_LIMIT_REACHED
without changing session/transcript state, and can be retried after capacity returns.
See [contact quota audit](../../contact-quota-audit-2026-09-11.md).
