...
**Status:** GraphQL-only consumer; worker protocol retained

**Evidence date:** 2026-07-22

## Decision

Administrator email templates, audit logs, and preview rendering are owned by `AdminMessagingModule`. Provider-facing batch delivery is owned by `MessagingDeliveryModule`. The frontend calls both modules directly through GraphQL; the former messaging and delivery consumer flags and all five Express admin-email endpoints were retired. `ADMIN_EMAIL_DELIVERY_SCHEDULER_ENABLED` remains the operational worker switch.

Both modules use database-backed `AdminAccessGuard`; no operation accepts organization context because the surface is intentionally global. Preview and enqueue mutations are CSRF-protected. Disabling the scheduler stops provider work without discarding committed intent.
...
Migration `041_admin_email_deliveries` remains a required pre-deploy gate. Operational recovery can disable the scheduler without discarding queued jobs; application recovery uses a release rollback rather than a second live frontend transport. Queued jobs and audit rows remain durable and require no data repair.
