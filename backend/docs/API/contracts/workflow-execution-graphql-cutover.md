...
**Status:** Operator GraphQL plus schedule/trigger/enrollment workers implemented; provider worker ownership remains
...
The NestJS commands `jobs:workflow-schedules`, `jobs:workflow-triggers`, and `jobs:workflow-enrollments` are bounded and inert unless explicitly invoked. They support optional ID-targeted execution for canaries, recover expired leases, fence stale attempts, redact stored failures, and preserve retained state transitions. The enrollment runner commits each database mutation or provider-intent snapshot together with safe execution logs and progress; waits release the claim at an absolute timestamp. No scheduler or deployment flag invokes the NestJS commands yet. The retained `WORKFLOW_ENROLLMENT_JOBS_ENABLED` gate continues to control legacy runtime execution until final configuration reconciliation.
...
`WorkflowExecutionModule` now owns those GraphQL operations plus `retryWorkflowSideEffect`. `WorkflowJobsModule` owns due-schedule dispatch, trigger fan-out, and ordered enrollment execution behind explicit one-shot entrypoints. The NestJS engine supports all canonical database, condition, wait, and provider-intent steps; provider calls remain outside it. Fresh PostgreSQL coverage proves exact status/type and due-work metrics, strict filters and bounded stable paging, safe error redaction, schema-level payload omission, tenant concealment, CSRF, dead-letter retry history, accepted-SID SMS reconciliation with correlated log persistence, single-winner schedule/trigger/enrollment claims under contention, ordered mutation/log progress, wait resumption, immutable email/SMS/webhook snapshots, expired-lease recovery, stale-attempt fencing, and redacted trigger dead letters. The legacy HTTP adapters remain available for rollback while provider delivery is transferred.
