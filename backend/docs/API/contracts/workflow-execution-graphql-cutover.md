...
The legacy singleton remains unused, but first-party trigger callsites no longer depend on it. Contact creation/update, actual bulk tag addition/removal, deal-stage movement, form submission, booking create/cancel/reschedule, invoice-paid transitions, and linked-contact contract completion now commit a canonical `workflow_triggers` event in the same transaction as their domain mutation. Scheduled workflows persist one tenant-owned contact and one absolute run time; a competing-safe dispatcher converts each due one-shot schedule into that same queue exactly once. A leased trigger worker atomically matches active workflows, evaluates trigger conditions, creates or reactivates enrollments, and marks the event complete. A separate enrollment scheduler invokes the existing step engine.

Both new scheduler phases require an intentional rollout: `WORKFLOW_TRIGGER_JOBS_ENABLED=true` enables event fan-out and `WORKFLOW_ENROLLMENT_JOBS_ENABLED=true` enables due enrollment execution. They default to disabled. This prevents a schema/code deployment from silently activating the previously dormant execution path.

The minute scheduler runs enabled workflow work through one ordered cycle: due one-shot schedules, trigger fan-out, enrollment execution, then provider side-effect delivery. The cycle preserves independent flags, so queued provider work can remain enabled while trigger or enrollment rollout is paused. Unit tests freeze flag defaults, disabled-phase behavior, and phase order.

The legacy provider boundary is now safe to characterize independently. Email, SMS, and outbound-webhook steps snapshot a durable `workflow_side_effect_outbox` intent before the enrollment advances. A scheduled worker leases those intents with `FOR UPDATE SKIP LOCKED`, applies provider-specific bounded recovery, records redacted dead letters, and writes provider IDs plus email/SMS logs only after provider acceptance. Twilio message creation gets one local attempt: a timeout, network failure, or other response without a known HTTP status moves directly to operator reconciliation, while a known rejection may retry. Cancellation terminates queued, retrying, dead-letter, and reconciliation-required intents; an already-running provider request may still record success, while a failed or lease-expired cancelled request becomes terminally cancelled without another attempt. `WORKFLOW_SIDE_EFFECT_JOBS_ENABLED=false` disables this worker; it is enabled by default and runs every minute.
