...
The retained contact transfer routes use the same topology. The legacy origin
always proxies only `GET /api/contacts/export/csv` and
`POST /api/contacts/import/csv` to the NestJS service identified by
`GRAPHQL_UPSTREAM_URL`; there is no duplicate Express implementation.
`CONTACT_TRANSFERS_UPSTREAM_TIMEOUT_MS` defaults to 30 seconds. The proxy
forwards only the host cookie, organization, CSRF, and request-ID headers and
returns only the required download/content/request headers.
