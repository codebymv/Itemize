...
The latest 2026-07-23 clean-schema checkpoint verifies 111 expected tables and 83 top-level migration markers. The complete retained baseline is 30 legacy suites with a 489-test pass. The 28 NestJS GraphQL/retained-HTTP suites pass 225 tests. In addition to the earlier authentication, CRM, scheduling, billing, messaging, workflow, sharing, and reputation contracts, the signature suite now proves authenticated upload/private delivery, public capability projection and first-view semantics, signer-field ownership rollback, sequential activation, document-wide decline revocation, competing terminal-action serialization, and leased signed-PDF completion.
