...
The latest 2026-07-23 clean-schema checkpoint verifies 111 expected tables and 88 top-level migration markers. Marker discovery now includes multiline `runMigrationOnce` calls, which exposed one previously uncounted existing marker. Migration 048 broadens durable shared-revocation delivery to every non-vault workspace kind. The retained suite currently passes 479 of 493 tests; 14 booking cases use calendar windows that are no longer in the future relative to the test date and fail before the Nest suite starts. The isolated workspace-content GraphQL suite passes all 18 tests, including list/note/whiteboard/wireframe sharing parity, wireframe read/create/update/delete parity, stale revision rejection, tenant concealment, public capability reads and old-token denial, token reuse/rotation, durable revocation plus shared/owner realtime worker delivery, mixed list/note/whiteboard/wireframe/vault position persistence, and cleanup on a disposable PostgreSQL schema.
