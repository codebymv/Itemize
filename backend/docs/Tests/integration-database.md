...
The latest 2026-07-19 run verified 98 expected tables and 73 top-level migration markers. The complete retained baseline is 30 legacy suites with 489 tests, including durable calendar-sync request coalescing, competing claims, retry/dead-lettering, direction dispatch, and external-busy replacement. The 11 NestJS GraphQL suites pass 98 tests; the added calendar-integration contract proves current-user/organization isolation, schema-level credential exclusion, validated settings, CSRF, idempotent durable enqueue/status, and scoped disconnect.
