...
describe('AdminOperationsService', () => {
  ...
  it('calculates an admin-only activation cohort with explicit denominators', async () => {
    repository.activationFunnel.mockResolvedValue({
      ...
      organizations_verified: 8,
      organizations_workspace_activated: 6,
      organizations_trial_started: 5,
      organizations_contact_created: 4,
      organizations_artifact_created: 5,
      organizations_sent: 4,
      organizations_advanced: 3,
      organizations_returned: 2,
      organizations_checkout_started: 2,
      organizations_subscription_activated: 1,
      trial_organizations_sent: 2,
      organizations_trial_to_paid: 1,
      median_hours_to_workspace: 0.75,
      median_hours_to_trial: 2,
      median_hours_to_contact: 3.5,
      median_hours_to_artifact: 4,
      median_hours_to_send: 5,
      median_hours_to_advance: 6,
      median_hours_to_checkout: 7,
      median_hours_to_subscription: 8.126,
    });
    await expect(service.activationFunnel()).resolves.toMatchObject({
      ...
      artifactCreationRate: 0.625,
      artifactToSendRate: 0.8,
      checkoutStartRate: 0.25,
      subscriptionActivationRate: 0.5,
      advanceRate: 0.75,
      returnRate: 0.5,
      trialToPaidRate: 0.5,
      medianHoursToSubscription: 8.13,
    });
