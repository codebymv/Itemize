...
const ACTIONS = new Set(['page', 'analytics']);
...
const publicLandingPagesEnabled = (environment = process.env) =>
...
const unavailableBody = () => ({
    success: false,
    error: {
        message: 'Landing page service is unavailable',
...
const createPublicLandingPagesProxy = ({
    action,
    environment = process.env,
    fetchImpl = global.fetch,
    logger = console,
} = {}) => {
    if (!ACTIONS.has(action)) throw new Error('Public landing pages proxy target is not allowed');
    const enabled = publicLandingPagesEnabled(environment);
    const baseUrl = enabled ? resolveBaseUrl(environment) : null;
    const configuredTimeout = Number(environment.PUBLIC_LANDING_PAGES_UPSTREAM_TIMEOUT_MS);
    ...
    return async (request, response, next) => {
        ...
        const target = new URL(
            `/api/pages/public/page/${encodeURIComponent(slug)}${action === 'analytics' ? '/analytics' : ''}`,
            baseUrl,
        );
        const method = action === 'page' ? 'GET' : 'POST';
        ...
        if (action === 'page') {
            ...
            // The page read consumes the visitor cookie and password header;
            // both stay inside our own origin-to-service hop.
            for (const name of ['cookie', 'x-page-password', 'referer']) {
        ...
        try {
            ...
            logger.error?.('Public landing pages proxy failed', {
                event: 'public_landing_pages_proxy_failed',
                action,
                requestId,
