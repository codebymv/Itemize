...
export class SignatureTemplatesResolver {
  ...
  @CsrfProtected() @OrganizationScoped() @Mutation(()=>SignatureTemplate) createSignatureTemplate(@Args('input')input:CreateSignatureTemplateInput,@Args('idempotencyKey')idempotencyKey:string):Promise<SignatureTemplate>{return this.service.create(this.organizationId(),this.userId(),input,idempotencyKey);}
  ...
  @CsrfProtected() @OrganizationScoped() @Mutation(()=>SignatureDocument) instantiateSignatureTemplate(@Args('id',{type:()=>Int})id:number,@Args('input')input:InstantiateSignatureTemplateInput,@Args('idempotencyKey')idempotencyKey:string):Promise<SignatureDocument>{return this.service.instantiate(this.organizationId(),this.userId(),id,input,idempotencyKey);}
