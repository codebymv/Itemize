...
const ACTIONS = new Set(['open', 'accept', 'decline']);

const publicEstimatesEnabled = (environment = process.env) =>
    environment.PUBLIC_ESTIMATES_NESTJS_ENABLED === 'true';
...
const createPublicEstimateProxy = ({
    action,
    environment = process.env,
    fetchImpl = global.fetch,
    logger = console,
} = {}) => {
    if (!ACTIONS.has(action)) throw new Error('Public estimate proxy target is not allowed');
    const enabled = publicEstimatesEnabled(environment);
    const baseUrl = enabled ? resolveBaseUrl(environment) : null;
    const configuredTimeout = Number(environment.PUBLIC_ESTIMATES_UPSTREAM_TIMEOUT_MS);
    ...
    return async (request, response, next) => {
        ...
        if (!baseUrl) {
            return response.status(503).json({
                success: false,
                error: { message: 'Public estimate service is unavailable', code: 'SERVICE_UNAVAILABLE' },
            });
        }
        ...
        if (!TOKEN.test(token)) {
            return response.status(404).json({
                success: false,
                error: { message: 'Estimate link is invalid or expired', code: 'NOT_FOUND' },
            });
        }
        const suffix = action === 'open' ? '' : `/${action}`;
        ...
        try {
            const upstream = await fetchImpl(
                new URL(`/api/public/estimates/${encodeURIComponent(token)}${suffix}`, baseUrl),
                {
                    method: action === 'open' ? 'GET' : 'POST',
                    headers,
                    redirect: 'error',
            ...
            logger.error?.('Public estimate proxy failed', {
                event: 'public_estimate_proxy_failed',
                action,
                requestId,
