...
import type { CreateConversationValues } from './conversations.repository';
...
export const conversationCreationFingerprint = (
  values: CreateConversationValues,
): string => createHash('sha256')
  .update(JSON.stringify(canonicalize(values)))
  .digest('hex');

export const conversationCreationKey = (value: string): string => {
