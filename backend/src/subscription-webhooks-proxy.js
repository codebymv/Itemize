...
const subscriptionWebhooksEnabled = (environment = process.env) =>
...
const unavailableBody = () => ({
    success: false,
    error: {
        message: 'Billing webhook service is unavailable',
...
const createSubscriptionWebhookProxy = ({
    environment = process.env,
    fetchImpl = global.fetch,
    logger = console,
} = {}) => {
    const enabled = subscriptionWebhooksEnabled(environment);
    const baseUrl = enabled ? resolveBaseUrl(environment) : null;
    const configuredTimeout = Number(environment.SUBSCRIPTION_WEBHOOKS_UPSTREAM_TIMEOUT_MS);
    ...
    return async (request, response, next) => {
        ...
        // The origin parses this path with express.raw, so req.body already
        // holds the exact signed bytes; the raw capture is the fallback.
        ...
        try {
            const upstream = await fetchImpl(
                new URL('/api/billing/webhook', baseUrl),
                {
                    method: 'POST',
                    headers,
                    body,
            ...
            logger.error?.('Subscription webhook proxy failed', {
                event: 'subscription_webhook_proxy_failed',
                requestId,
