...
const transactionalEmailAssetOrigin = (environment = process.env) => {
  const fallback = 'https://itemize.cloud';
  try {
    const configured = new URL(
      environment.EMAIL_ASSET_ORIGIN || environment.PROD_URL || fallback,
    );
...
const brandedTransactionalEmail = input => {
  const origin = String(input.assetOrigin).replace(/\/$/, '');
