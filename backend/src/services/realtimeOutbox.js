...
const CHAT_SESSION_TOKEN_PATTERN = /^cs_[0-9a-f]{48}$/;
...
function validateRealtimeEvent(event) {
  ...
  if (!['list', 'note', 'whiteboard', 'chat_session'].includes(event.aggregateType)) {
  ...
  if (event.channel === 'chat_session' && !CHAT_SESSION_TOKEN_PATTERN.test(event.recipientKey)) {
    throw new Error('Realtime chat recipient must be a session capability');
  }
  if (
    !['user_canvas', 'chat_session'].includes(event.channel)
    && !SHARE_TOKEN_PATTERN.test(event.recipientKey)
  ) {
  ...
  if (
    ...
    || (event.channel === 'chat_session' && event.aggregateType !== 'chat_session')
    || (
      !['shared_note', 'shared_whiteboard', 'chat_session'].includes(event.channel)
