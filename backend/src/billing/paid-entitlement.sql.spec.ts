import { paidEntitlementSql } from './paid-entitlement.sql';

describe('paidEntitlementSql', () => {
  it('matches the paid-plan and live-billing contract', () => {
  ...
  it('rejects an unsafe SQL alias', () => {
    expect(() => paidEntitlementSql('org; DROP TABLE users')).toThrow(
      'Unsafe organization SQL alias',
    );
  });
});
