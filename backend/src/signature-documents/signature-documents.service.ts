...
export class SignatureDocumentsService {
  ...
  async list(organizationId: number, filter: SignatureDocumentFilterInput = {}, page: PageInput = new PageInput()): Promise<SignatureDocumentPage> {
    ...
    const search = this.optionalText(filter.search, 'search', 255) ?? undefined;
    const result = await this.repository.findPage({ organizationId, statuses, search, pageSize: normalized.pageSize, offset: normalized.offset });
    return {
      nodes: result.rows.map((row) => this.document(row)),
  ...
  private statuses(filter: SignatureDocumentFilterInput): SignatureDocumentStatus[] | undefined {
    const values = filter.statuses?.length ? filter.statuses : filter.status ? [filter.status] : undefined;
    return values ? [...new Set(values)] : undefined;
  }
  private stats(row: { total: number | string; invalid: number | string; draft: number | string; active: number | string; completed: number | string }): SignatureDocumentStats {
    return { total:Number(row.total),invalid:Number(row.invalid),draft:Number(row.draft),active:Number(row.active),completed:Number(row.completed) };
  }
  private document(row: SignatureDocumentRow): SignatureDocument { return { id:row.id,organizationId:row.organization_id,title:row.title,documentNumber:row.document_number,description:row.description,message:row.message,status:row.status,recipientCount:Number(row.recipient_count),routingMode:row.routing_mode,templateId:row.template_id,expirationDays:row.expiration_days,expiresAt:row.expires_at,senderName:row.sender_name,senderEmail:row.sender_email,createdById:row.created_by,sentAt:row.sent_at,completedAt:row.completed_at,hasFile:row.has_file,hasSignedFile:row.has_signed_file,fileName:row.file_name,fileType:row.file_type,fileSize:row.file_size===null?null:Number(row.file_size),pageCount:row.page_count??null,deliveryState:row.delivery_state??null,completionState:row.completion_state??null,createdAt:row.created_at,updatedAt:row.updated_at }; }
  private recipient(row: SignatureRecipientRow): SignatureRecipient { return { id:row.id,documentId:row.document_id,organizationId:row.organization_id,contactId:row.contact_id,name:row.name,email:row.email,signingOrder:row.signing_order,roleName:row.role_name,routingStatus:row.routing_status,status:row.status,sentAt:row.sent_at,viewedAt:row.viewed_at,signedAt:row.signed_at,declinedAt:row.declined_at,declineReason:row.decline_reason,identityMethod:row.identity_method,identityVerifiedAt:row.identity_verified_at,deliveryState:row.delivery_state??null }; }
