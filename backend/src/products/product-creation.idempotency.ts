...
import type { ProductValues } from './products.repository';
...
export const productCreationFingerprint = (values: ProductValues): string =>
  createHash('sha256')
    .update(JSON.stringify(canonicalize(values)))
    .digest('hex');

export const productCreationKey = (value: string): string => {
