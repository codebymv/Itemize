...
import { BRAND_BLUE, BRAND_BLUE_HOVER } from '../common/brand';
...
const page = (state: 'ready' | 'unsubscribed' | 'invalid', token: string): string => {
  ...
  const action = state === 'ready'
    ...
    : '';
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${copy.title}</title><style>body{margin:0;background:#f1f5f9;color:#0f172a;font-family:Raleway,Segoe UI,Arial,sans-serif}main{box-sizing:border-box;max-width:560px;margin:10vh auto;padding:32px 28px;background:#fff;border:1px solid #e2e8f0;border-top:4px solid ${BRAND_BLUE};border-radius:12px;box-shadow:0 12px 30px rgba(15,23,42,.08)}h1{margin:0 0 12px;font-size:26px}p{margin:0 0 24px;color:#475569;line-height:1.6}button{border:0;border-radius:8px;background:${BRAND_BLUE};color:#fff;padding:12px 18px;font:inherit;font-weight:700;cursor:pointer}button:hover{background:${BRAND_BLUE_HOVER}}@media(max-width:620px){main{margin:24px 16px;padding:28px 22px}}</style></head><body><main><h1>${copy.title}</h1><p>${copy.detail}</p>${action}</main></body></html>`;
};
